...
bool LoadBinaryResource(int binaryId, DWORD& dwSize, LPBYTE& pBytes)
{
    ...
    if (hRes)
    {
        ...
        if (hGlob)
        {
            ...
            if (dwSize > 0 && pBytes) return true;
        }
    }
...
int GetResourceId(const char* resource_name)
{
    ...
    static struct _resource_map
    {
        ...
        int id = -1;
    } resource_map[] = {{"Logo.png", IDS_LOGO_PNG}, {"binding.html", IDS_BINDING_HTML}};

    for (int i = 0; i < sizeof(resource_map) / sizeof(_resource_map); ++i)
    {
        if (!_stricmp(resource_map[i].name.c_str(), resource_name)) return resource_map[i].id;
    }
