...
#ifndef __MAIN_MESSAGE_LOOP_H_
#define __MAIN_MESSAGE_LOOP_H_
#pragma once
...
#define CURRENTLY_ON_MAIN_THREAD() MainMessageLoop::Get()->RunsTasksOnCurrentThread()

#define REQUIRE_MAIN_THREAD() DCHECK(CURRENTLY_ON_MAIN_THREAD())

#define MAIN_POST_TASK(task) MainMessageLoop::Get()->PostTask(task)

#define MAIN_POST_CLOSURE(closure) MainMessageLoop::Get()->PostClosure(closure)
    ...
    static void Destruct(const T* x)
    {
        ...
        else
        {
            MainMessageLoop::Get()->PostClosure(
                base::BindOnce(&DeleteOnMainThread::Destruct<T>, base::Unretained(x)));
        }
    }
};
