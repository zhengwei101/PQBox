...
public:
    ...
    static std::string FileDigest(const std::wstring& file)
    {
    ...
    static std::string BufferDigest(unsigned char* buf, int buf_len)
    {
        if (!buf || buf_len <= 0)
        {
            return "";
    ...
    static std::string BufferDigest(const std::string& buf)
    {
        Md5 md5(buf);
        return md5.toString();
    }
    ...
    void update(const std::string& str)
    {
        ...
        update((const unsigned char*)str.c_str(), size);
    }
...
private:
    ...
    //Convert unsigned char array to hex std::string.
    std::string bytesToHexString(const unsigned char* input, size_type length)
    {
        std::string str;
        size_t len(length);
        str.reserve(len << 1);
