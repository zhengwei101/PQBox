...
typedef HRESULT(WINAPI* LPGetProcessDpiAwareness)(
    _In_ HANDLE hprocess, _Out_ PROCESS_DPI_AWARENESS* value);
    ...
    if (IsWindows8Point1OrGreater())
    {
        ...
        if (hModule)
        {
            LPGetProcessDpiAwareness GetProcessDpiAwareness =
                (LPGetProcessDpiAwareness)GetProcAddress(
                    hModule, "GetProcessDpiAwareness");
            if (GetProcessDpiAwareness)
            {
                HANDLE hProcess = OpenProcess(
                    PROCESS_ALL_ACCESS, false, GetCurrentProcessId());
    ...
    if (IsWindows8Point1OrGreater())
    {
        ...
        if (hModule)
        {
            LPSetProcessDpiAwareness SetProcessDpiAwareness =
                (LPSetProcessDpiAwareness)GetProcAddress(
                    hModule, "SetProcessDpiAwareness");
            if (SetProcessDpiAwareness &&
                SetProcessDpiAwareness(Awareness) == S_OK)
            {
