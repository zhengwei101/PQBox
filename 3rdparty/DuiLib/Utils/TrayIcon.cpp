...
void CTrayIcon::CreateTrayIcon(HWND _RecvHwnd,
                               UINT _IconIDResource,
                               LPCTSTR _ToolTipText,
                               UINT _Message)
{
    ...
    if (_Message != 0)
        m_uMessage = _Message;
    m_hIcon = LoadIcon(CPaintManagerUI::GetInstance(),
                       MAKEINTRESOURCE(_IconIDResource));
...
{
    HICON hIcon =
        LoadIcon(CPaintManagerUI::GetInstance(), MAKEINTRESOURCE(_IDResource));
    return SetIcon(hIcon);
}
