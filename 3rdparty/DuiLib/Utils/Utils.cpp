...
bool CStdPtrArray::Remove(int iIndex)
{
    ...
    if (iIndex < --m_nCount)
        ::CopyMemory(
            m_ppVoid + iIndex, m_ppVoid + iIndex + 1, (m_nCount - iIndex) * sizeof(LPVOID));
    return true;
}
...
bool CStdValArray::Remove(int iIndex)
{
    if (iIndex < 0 || iIndex >= m_nCount)
        return false;
    if (iIndex < --m_nCount)
        ::CopyMemory(m_pVoid + (iIndex * m_iElementSize),
                     m_pVoid + ((iIndex + 1) * m_iElementSize),
...
{
    size_t length = _vsntprintf(nullptr, 0, pstrFormat, args);
    TCHAR* szBuffer = new TCHAR[length + 1];
    int nLen = _vsntprintf(szBuffer, length + 1, pstrFormat, args);
    Assign(szBuffer);
    delete szBuffer;
    szBuffer = nullptr;
    return nLen;
