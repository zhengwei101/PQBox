...
void CShadowUI::Create(CPaintManagerUI* pPaintManager)
{
    ...
    _ASSERT(GetShadowMap().find(hParentWnd) ==
            GetShadowMap().end()); // Only one shadow for each window
    ...
    if (!(WS_VISIBLE & lParentStyle)) // Parent invisible
        m_Status = SS_ENABLED;
    else if ((WS_MAXIMIZE | WS_MINIMIZE) &
             lParentStyle) // Parent visible but does not need shadow
...
LRESULT CALLBACK CShadowUI::ParentProc(HWND hwnd,
                                       UINT uMsg,
                                       WPARAM wParam,
                                       LPARAM lParam)
{
    _ASSERT(GetShadowMap().find(hwnd) !=
            GetShadowMap().end()); // Shadow must have been attached

    CShadowUI* pThis = GetShadowMap()[hwnd];
    if (pThis->IsDisableShadow())
    {
        return CallWindowProc(
            (WNDPROC)pThis->m_OriParentProc, hwnd, uMsg, wParam, lParam);
    }
    ...
    case WM_WINDOWPOSCHANGED: {
        ...
        if (pThis->m_bIsImageMode)
        {
            SetWindowPos(pThis->m_hWnd,
                         hwnd,
                         WndRect.left - pThis->m_nSize,
                         WndRect.top - pThis->m_nSize,
                         0,
                         0,
        ...
        else
        {
            SetWindowPos(pThis->m_hWnd,
                         hwnd,
                         WndRect.left + pThis->m_nxOffset - pThis->m_nSize,
                         WndRect.top + pThis->m_nyOffset - pThis->m_nSize,
                         0,
                         0,
                         SWP_NOSIZE | SWP_NOACTIVATE);
        }
    ...
    case WM_SIZE:
        if (pThis->m_Status & SS_ENABLED)
        {
            ...
            else if (pThis->m_Status & SS_PARENTVISIBLE)
            {
                ...
                // the window region would never be updated until WM_PAINT was
                // sent. So do not Update() until next WM_PAINT is received in
                // this case
                if (LOWORD(lParam) > LOWORD(pThis->m_WndSize) ||
                    HIWORD(lParam) > HIWORD(pThis->m_WndSize))
                    pThis->m_bUpdate = true;
                else
                    pThis->Update(hwnd);
    ...
    case WM_PAINT: {
        ...
        // In some cases of sizing, the up-right corner of the parent window
        // region would not be properly updated Update() again when sizing is
        // finished
    ...
    case WM_NCDESTROY:
        GetShadowMap().erase(
            hwnd); // Remove this window and shadow from the map
        break;
    }
...
void GetLastErrorMessage()
{
    ...
    // Display the string.
    // MessageBox(NULL, (const wchar_t*)lpMsgBuf, L"GetLastError", MB_OK |
    // MB_ICONINFORMATION);
...
void CShadowUI::Update(HWND hParent)
{
    ...
    HBITMAP hbitmap =
        CreateDIBSection(NULL, &bmi, DIB_RGB_COLORS, (void**)&pvBits, NULL, 0);
    if (hbitmap == NULL)
    {
        GetLastErrorMessage();
    }
    ...
    if (m_bIsImageMode)
    {
        ...
        const TDrawInfo* pDrawInfo =
            m_pManager->GetDrawInfo(m_sShadowImage, _T(""));
        if (pDrawInfo)
        {
            const TImageInfo* data =
                m_pManager->GetImageEx(pDrawInfo->sImageName,
                                       pDrawInfo->sResType,
                                       pDrawInfo->dwMask,
                                       pDrawInfo->bHSL,
                                       pDrawInfo->bGdiplus);
            ...
            CRenderEngine::DrawImage(hMemDC,
                                     data->hBitmap,
                                     rcPaint,
                                     rcPaint,
                                     rcBmpPart,
    ...
    BOOL bRet = ::UpdateLayeredWindow(m_hWnd,
                                      NULL,
                                      &ptDst,
                                      &WndSize,
                                      hMemDC,
...
void CShadowUI::MakeShadow(UINT32* pShadBits, HWND hParent, RECT* rcParent)
{
    ...
    // Apply morphologic erosion to shrink it into the size (ShadowWndSize -
    // Sharpness) Apply modified (with blur effect) morphologic dilation to make
    // the blurred border The algorithm is optimized by assuming parent window
    // is just "one piece" and without "wholes" on it

    // Get the region of parent window,
    HRGN hParentRgn = CreateRectRgn(0,
                                    0,
                                    abs(rcParent->right - rcParent->left),
                                    abs(rcParent->bottom - rcParent->top));
    GetWindowRgn(hParent, hParentRgn);

    // Determine the Start and end point of each horizontal scan line
    SIZE szParent = {rcParent->right - rcParent->left,
    ...
    int(*ptAnchorsOri)[2] =
        new int[szParent.cy]
               [2]; // anchor points, will not modify during erosion
    ...
    int(*ptAnchorsTmp)[2] =
        new int[nAnchors + 2][2]; // Store the result of erosion
    ...
    for (int i = 0; i < m_nSharpness - m_nSize; i++)
    {
        ...
        for (int j = 1; j < nAnchors + 1; j++)
        {
            ptAnchorsTmp[j][0] =
                max(ptAnchors[j - 1][0],
                    max(ptAnchors[j][0], ptAnchors[j + 1][0])) +
                1;
            ptAnchorsTmp[j][1] =
                min(ptAnchors[j - 1][1],
                    min(ptAnchors[j][1], ptAnchors[j + 1][1])) -
                1;
        }
    ...
    ptAnchors += (m_nSize < 0 ? -m_nSize : 0) +
                 1; // now coordinates in ptAnchors are same as in shadow window
    // Generate the kernel
    ...
    for (int i = 0; i <= 2 * nKernelSize; i++)
    {
        ...
        {
            double dLength =
                sqrt((i - nKernelSize) * (i - nKernelSize) +
                     (j - nKernelSize) * (double)(j - nKernelSize));
            if (dLength < nCenterSize)
                *pKernelIter =
                    m_nDarkness << 24 | PreMultiply(m_Color, m_nDarkness);
    ...
    for (int i = nKernelSize; i < szShadow.cy - nKernelSize; i++)
    {
        ...
        if (ptAnchors[i][0] < ptAnchors[i][1])
        {
            // Start of line
            for (j = ptAnchors[i][0];
                 j < min(max(ptAnchors[i - 1][0], ptAnchors[i + 1][0]) + 1,
                         ptAnchors[i][1]);
                 j++)
            {
                for (int k = 0; k <= 2 * nKernelSize; k++)
                {
                    UINT32* pPixel =
                        pShadBits +
                        (szShadow.cy - i - 1 + nKernelSize - k) * szShadow.cx +
                        j - nKernelSize;
    ...
    for (int i = min(nKernelSize, max(m_nSize - m_nyOffset, 0));
         i < max(szShadow.cy - nKernelSize,
                 min(szParent.cy + m_nSize - m_nyOffset,
                     szParent.cy + 2 * m_nSize));
         i++)
    {
        UINT32* pLine = pShadBits + (szShadow.cy - i - 1) * szShadow.cx;
        // Line is not covered by parent window
        if (i - m_nSize + m_nyOffset < 0 ||
            i - m_nSize + m_nyOffset >= szParent.cy)
        {
        ...
        else
        {
            for (int j = ptAnchors[i][0];
                 j < min(ptAnchorsOri[i - m_nSize + m_nyOffset][0] + m_nSize -
                             m_nxOffset,
                         ptAnchors[i][1]);
                 j++)
                *(pLine + j) = clCenter;
            for (int j = max(ptAnchorsOri[i - m_nSize + m_nyOffset][0] +
                                 m_nSize - m_nxOffset,
                             0);
                 j < min(ptAnchorsOri[i - m_nSize + m_nyOffset][1] + m_nSize -
                             m_nxOffset,
                         szShadow.cx);
                 j++)
                *(pLine + j) = 0;
            for (int j = max(ptAnchorsOri[i - m_nSize + m_nyOffset][1] +
                                 m_nSize - m_nxOffset,
                             ptAnchors[i][0]);
                 j < ptAnchors[i][1];
                 j++)
    ...
    if (m_hWnd)
    {
        ...
        else
        {
            ...
            if ((WS_VISIBLE & lParentStyle) &&
                !((WS_MAXIMIZE | WS_MINIMIZE) & lParentStyle))
            {
                ::ShowWindow(m_hWnd, SW_SHOWNOACTIVATE);
...
{
    if (NewXOffset > 35 || NewXOffset < -35 || NewYOffset > 35 ||
        NewYOffset < -35)
        return false;
...
{
    if (rcCorner.left < 0 || rcCorner.top < 0 || rcCorner.right < 0 ||
        rcCorner.bottom < 0)
        return false;
