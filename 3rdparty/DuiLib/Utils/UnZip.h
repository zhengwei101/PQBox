...
ZIPAPI HZIP OpenZip(const TCHAR *fn, const char *password);
ZIPAPI HZIP OpenZip(void *z, unsigned int len, const char *password);
ZIPAPI HZIP OpenZipHandle(HANDLE h, const char *password);
...
ZIPAPI ZRESULT GetZipItem(HZIP hz, int index, ZIPENTRY *ze);
...
ZIPAPI ZRESULT FindZipItem(HZIP hz, const TCHAR *name, bool ic, int *index, ZIPENTRY *ze);
// FindZipItem - finds an item by name. ic means 'insensitive to case'.
// It returns the index of the item, and returns information about it.
// If nothing was found, then index is set to -1 and the function returns
// an error code.

ZIPAPI ZRESULT UnzipItem(HZIP hz, int index, const TCHAR *fn);
ZIPAPI ZRESULT UnzipItem(HZIP hz, int index, void *z, unsigned int len);
ZIPAPI ZRESULT UnzipItemHandle(HZIP hz, int index, HANDLE h);
