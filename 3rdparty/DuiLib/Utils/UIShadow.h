...
protected:
    enum ShadowStatus
    {
        ...
        SS_PARENTVISIBLE = 1 << 2 // Parent window is visible, if not, the above
                                  // one is always false
    };

    static bool s_bHasInit;

    CPaintManagerUI* m_pManager = nullptr; // 父窗体的CPaintManagerUI，用来获取素材资源和父窗体句柄
    ...
    // Set this to true if the shadow should not be update until next WM_PAINT
    // is received
    ...
    // relative to the parent window, at center of both windows (not top-left
    // corner), signed
    signed char m_nxOffset = 0;
    signed char m_nyOffset = 0;

    // Restore last parent window size, used to determine the update strategy
