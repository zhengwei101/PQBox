...
void CScrollBarUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    if (m_bHorizontal)
    {
        ...
        if (cx > cxyFixed.cy)
        {
            ...
            {
                int cxThumb =
                    cx * (rc.right - rc.left) / (m_nRange + rc.right - rc.left);
                if (cxThumb < cxyFixed.cy)
                    cxThumb = cxyFixed.cy;

                m_rcThumb.left = m_nScrollPos * (cx - cxThumb) / m_nRange +
                                 m_rcButton1.right;
        ...
        else
        {
            ...
            if (cxButton > cxyFixed.cy)
                cxButton = cxyFixed.cy;
            m_rcButton1.left = rc.left;
            m_rcButton1.top = rc.top;
            if (m_bShowButton1)
            {
    ...
    else
    {
        ...
        if (cy > cxyFixed.cx)
        {
            ...
            {
                int cyThumb =
                    cy * (rc.bottom - rc.top) / (m_nRange + rc.bottom - rc.top);
                if (cyThumb < cxyFixed.cx)
                    cyThumb = cxyFixed.cx;

                float value = (m_nScrollPos * 1.0f / m_nRange) * (cy - cyThumb);
                m_rcThumb.top = static_cast<int>(value) + m_rcButton1.bottom;
        ...
        else
        {
            ...
            if (cyButton > cxyFixed.cx)
                cyButton = cxyFixed.cx;
            m_rcButton1.left = rc.left;
            m_rcButton1.top = rc.top;
            if (m_bShowButton1)
            {
...
void CScrollBarUI::DoEvent(TEventUI& event)
{
            ...
            if (!m_bHorizontal)
            {
                if (event.ptMouse.y < m_rcThumb.top)
                {
                    if (m_pOwner)
                        ...
                        SetScrollPos(m_nScrollPos + m_rcItem.top -
                                     m_rcItem.bottom);
                }
            ...
            else
            {
                if (event.ptMouse.x < m_rcThumb.left)
                {
                    if (m_pOwner)
                        ...
                        SetScrollPos(m_nScrollPos + m_rcItem.left -
                                     m_rcItem.right);
                }
    ...
    if (event.Type == UIEVENT_MOUSEMOVE)
    {
        if ((m_uThumbState & UISTATE_CAPTURED) != 0)
        {
            if (!m_bHorizontal)
            {
                int fMouseRange = (event.ptMouse.y - m_ptLastMouse.y) * m_nRange;
                int nBtnSize = 0;
                if (GetShowButton1())
                    nBtnSize += m_cxyFixed.cx;
                if (GetShowButton2())
                    nBtnSize += m_cxyFixed.cx;
                int vRange = m_rcItem.bottom - m_rcItem.top -
                             (m_rcThumb.bottom - m_rcThumb.top) - nBtnSize;
                if (vRange != 0)
                {
            ...
            {
                INT32 fMouseRange = (event.ptMouse.x - m_ptLastMouse.x) * m_nRange;
                int nBtnSize = 0;
                if (GetShowButton1())
                    nBtnSize += m_cxyFixed.cy;
                if (GetShowButton2())
                    nBtnSize += m_cxyFixed.cy;
                int hRange = m_rcItem.right - m_rcItem.left - m_rcThumb.right +
                             m_rcThumb.left - nBtnSize;
                if (hRange != 0)
                    m_nLastScrollOffset = fMouseRange / abs(hRange);
            }
        }
        else
        {
    ...
    if (event.Type == UIEVENT_TIMER && event.wParam == DEFAULT_TIMERID)
    {
        ...
        if ((m_uThumbState & UISTATE_CAPTURED) != 0)
        {
            if (!m_bHorizontal)
            {
                if (m_pOwner)
                    m_pOwner->SetScrollPos(
                        CDuiSize(m_pOwner->GetScrollPos().cx,
            ...
            else
            {
                if (m_pOwner)
                    m_pOwner->SetScrollPos(
                        CDuiSize(m_nLastScrollPos + m_nLastScrollOffset,
...
bool CScrollBarUI::DoPaint(HDC hDC,
                           const RECT& rcPaint,
...
void CScrollBarUI::PaintButton1(HDC hDC)
{
    ...
    int d1 = MulDiv(m_rcButton1.left - m_rcItem.left,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d2 = MulDiv(m_rcButton1.top - m_rcItem.top,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d3 = MulDiv(m_rcButton1.right - m_rcItem.left,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d4 = MulDiv(m_rcButton1.bottom - m_rcItem.top,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    ...
    else if ((m_uButton1State & UISTATE_HOT) != 0)
    {
        ...
        {
            if (!DrawImage(
                    hDC, (LPCTSTR)m_sButton1HotImage, (LPCTSTR)m_sImageModify))
            {
            }
            else
                return;
    ...
    {
        if (!DrawImage(
                hDC, (LPCTSTR)m_sButton1NormalImage, (LPCTSTR)m_sImageModify))
        {
        }
        else
            return;
...
void CScrollBarUI::PaintButton2(HDC hDC)
{
    ...
    if (!IsEnabled())
        ...
        m_uButton2State &= ~UISTATE_DISABLED;
    int d1 = MulDiv(m_rcButton2.left - m_rcItem.left,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d2 = MulDiv(m_rcButton2.top - m_rcItem.top,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d3 = MulDiv(m_rcButton2.right - m_rcItem.left,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d4 = MulDiv(m_rcButton2.bottom - m_rcItem.top,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    m_sImageModify.Empty();
    m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), d1, d2, d3, d4);
    ...
    else if ((m_uButton2State & UISTATE_HOT) != 0)
    {
        ...
        {
            if (!DrawImage(
                    hDC, (LPCTSTR)m_sButton2HotImage, (LPCTSTR)m_sImageModify))
            {
            }
            else
                return;
    ...
    {
        if (!DrawImage(
                hDC, (LPCTSTR)m_sButton2NormalImage, (LPCTSTR)m_sImageModify))
        {
        }
        else
            return;
...
{
    if (m_rcThumb.left == 0 && m_rcThumb.top == 0 && m_rcThumb.right == 0 &&
        m_rcThumb.bottom == 0)
        return;
    if (!IsEnabled())
        m_uThumbState |= UISTATE_DISABLED;
        ...
        m_uThumbState &= ~UISTATE_DISABLED;
    int d1 = MulDiv(m_rcThumb.left - m_rcItem.left,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d2 = MulDiv(m_rcThumb.top - m_rcItem.top,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d3 = MulDiv(m_rcThumb.right - m_rcItem.left,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    int d4 = MulDiv(m_rcThumb.bottom - m_rcItem.top,
                    100,
                    GetManager()->GetDPIObj()->GetScale());
    m_sImageModify.Empty();
    m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), d1, d2, d3, d4);
    ...
    else if ((m_uThumbState & UISTATE_PUSHED) != 0)
    {
        ...
        {
            if (!DrawImage(
                    hDC, (LPCTSTR)m_sThumbPushedImage, (LPCTSTR)m_sImageModify))
            {
            }
            else
                return;
    ...
    else if ((m_uThumbState & UISTATE_HOT) != 0)
    {
        ...
        {
            if (!DrawImage(
                    hDC, (LPCTSTR)m_sThumbHotImage, (LPCTSTR)m_sImageModify))
            {
            }
            else
                return;
    ...
    {
        if (!DrawImage(
                hDC, (LPCTSTR)m_sThumbNormalImage, (LPCTSTR)m_sImageModify))
        {
        }
        else
            return;
...
void CScrollBarUI::PaintRail(HDC hDC)
{
    ...
    if (!m_bHorizontal)
    {
        int d1 = MulDiv(m_rcThumb.left - m_rcItem.left,
                        100,
                        GetManager()->GetDPIObj()->GetScale());
        int d2 = MulDiv((m_rcThumb.top + m_rcThumb.bottom) / 2 - m_rcItem.top -
                            m_cxyFixed.cx / 2,
                        100,
                        GetManager()->GetDPIObj()->GetScale());
        int d3 = MulDiv(m_rcThumb.right - m_rcItem.left,
                        100,
                        GetManager()->GetDPIObj()->GetScale());
        int d4 = MulDiv((m_rcThumb.top + m_rcThumb.bottom) / 2 - m_rcItem.top +
                            m_cxyFixed.cx - m_cxyFixed.cx / 2,
                        100,
                        GetManager()->GetDPIObj()->GetScale());
        m_sImageModify.SmallFormat(_T("dest='%d,%d,%d,%d'"), d1, d2, d3, d4);
    }
    ...
    {
        int d1 = MulDiv((m_rcThumb.left + m_rcThumb.right) / 2 - m_rcItem.left -
                            m_cxyFixed.cy / 2,
                        100,
                        GetManager()->GetDPIObj()->GetScale());
        int d2 = MulDiv(m_rcThumb.top - m_rcItem.top,
                        100,
                        GetManager()->GetDPIObj()->GetScale());
        int d3 = MulDiv((m_rcThumb.left + m_rcThumb.right) / 2 - m_rcItem.left +
                            m_cxyFixed.cy - m_cxyFixed.cy / 2,
                        100,
                        GetManager()->GetDPIObj()->GetScale());
        int d4 = MulDiv(m_rcThumb.bottom - m_rcItem.top,
                        100,
    ...
    else if ((m_uThumbState & UISTATE_PUSHED) != 0)
    {
        ...
        {
            if (!DrawImage(
                    hDC, (LPCTSTR)m_sRailPushedImage, (LPCTSTR)m_sImageModify))
            {
            }
            else
                return;
    ...
    else if ((m_uThumbState & UISTATE_HOT) != 0)
    {
        ...
        {
            if (!DrawImage(
                    hDC, (LPCTSTR)m_sRailHotImage, (LPCTSTR)m_sImageModify))
            {
            }
            else
                return;
    ...
    {
        if (!DrawImage(
                hDC, (LPCTSTR)m_sRailNormalImage, (LPCTSTR)m_sImageModify))
        {
        }
        else
            return;
