...
bool CUIAnimation::StartAnimation(int nElapse,
                                  int nTotalFrame,
                                  int nAnimationID /*= 0*/,
                                  bool bLoop /* = FALSE*/)
{
    CAnimationData* pData = GetAnimationDataByID(nAnimationID);
    if (NULL != pData || nElapse <= 0 || nTotalFrame <= 0 || NULL == m_pControl)
    {
        ASSERT(FALSE);
        return FALSE;
    }

    CAnimationData* pAnimation =
        new CAnimationData(nElapse, nTotalFrame, nAnimationID, bLoop);
    if (NULL == pAnimation)
        return FALSE;
...
void CUIAnimation::StopAnimation(int nAnimationID /*= 0*/)
{
    ...
    if (nAnimationID != 0)
    {
        ...
        if (pData)
        {
            ...
            m_pImp->m_arAnimations.erase(
                std::remove(m_pImp->m_arAnimations.begin(),
    ...
    {
        for (size_t i = 0; i < m_pImp->m_arAnimations.size(); ++i)
        {
            CAnimationData* pData = m_pImp->m_arAnimations[i];
            if (pData)
            {
                m_pControl->GetManager()->KillTimer(
                    m_pControl, pData->m_nAnimationID);
...
void CUIAnimation::OnAnimationStart(int nAnimationID, bool bFirstLoop)
{

}

void CUIAnimation::OnAnimationStep(int nTotalFrame, int nCurFrame, int nAnimationID)
{

}

void CUIAnimation::OnAnimationStop(int nAnimationID)
{
