...
{
    if (m_sBkImage == pStrImage || NULL == pStrImage)
        return;
...
void CGifAnimUI::InitGifImage()
{
    ...
    if (m_nFrameCount > 1)
    {
        ...
        m_pGifImage->GetPropertyItem(
            PropertyTagFrameDelay, nSize, m_pPropertyItem);
    }
...
void CGifAnimUI::DrawFrame(HDC hDC)
{
    ...
    graphics.DrawImage(m_pGifImage,
                       Gdiplus::REAL(m_rcItem.left),
