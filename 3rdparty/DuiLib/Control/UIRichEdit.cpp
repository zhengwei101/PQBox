...
HRESULT
CreateHost(CRichEditUI* re, const CREATESTRUCT* pcs, CTxtWinHost** pptec)
{
    HRESULT hr = E_FAIL;
...
void CTxtWinHost::TxScrollWindowEx(INT dx,
                                   INT dy,
                                   LPCRECT lprcScroll,
                                   LPCRECT lprcClip,
                                   HRGN hrgnUpdate,
...
void CTxtWinHost::SetAllowBeep(BOOL allowBeep)
{
...
BOOL CTxtWinHost::DoSetCursor(RECT* prc, POINT* pt)
{
    ...
    if (PtInRect(&rc, *pt))
    {
        ...
        pserv->OnTxSetCursor(DVASPECT_CONTENT,
                             -1,
                             NULL,
                             NULL,
                             m_re->GetManager()->GetPaintDC(),
                             NULL,
                             prcClient,
                             pt->x,
                             pt->y);
...
{
    if (_tcscmp(pstrName, DUI_CTR_RICHEDIT) == 0)
...
void CRichEditUI::SetTransparent(bool bTransparent)
{
    ...
    if (m_pTwh)
        m_pTwh->SetTransparent(bTransparent);
}
...
void CRichEditUI::DoEvent(TEventUI& event)
{
    ...
    {
        if (m_pTwh)
            m_pTwh->NeedFreshCaret();
    }
    else if (event.Type == UIEVENT_SETFOCUS)
    {
        if (m_pTwh)
        {
            if (FALSE == m_bBlockTextOnFocus)
            {
                m_pTwh->OnTxInPlaceActivate(NULL);
    ...
    else if (event.Type == UIEVENT_TIMER)
    {
        if (event.wParam == DEFAULT_TIMERID)
        {
                ...
                m_pTwh->IsShowCaret())
            {
                ...
                RECT rcCaret = {ptCaret.x,
                                ptCaret.y,
                                ptCaret.x + m_pTwh->GetCaretWidth(),
                                ptCaret.y + m_pTwh->GetCaretHeight()};
                RECT rcTemp = rcCaret;
                if (!::IntersectRect(&rcCaret, &rcTemp, &m_rcItem))
                    return;
...
void CRichEditUI::SetBlockTextOnFocus(bool bBlock)
{
