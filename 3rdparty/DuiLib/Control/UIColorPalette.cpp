 ...
 */
#define _HSLToRGB(h, s, l)                                                     \
    (0xFF << 24 | HSLToRGB((float)h / 360.0f, (float)s / 200.0f, l / 200.0f))
...
DWORD CColorPaletteUI::GetSelectColor()
{
    ...
    return 0xFF << 24 | GetRValue(dwColor) << 16 | GetGValue(dwColor) << 8 |
           GetBValue(dwColor);
}
...
void CColorPaletteUI::SetBarHeight(int nHeight)
{
    ...
    {
        nHeight =
            150; //限制最大高度，由于当前设计，nheight超出190，程序会因越界访问崩溃
    }
...
void CColorPaletteUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    m_ptLastPalletMouse.x =
        m_nCurH * (m_rcItem.right - m_rcItem.left) / 360 + m_rcItem.left;
    m_ptLastPalletMouse.y =
        (200 - m_nCurB) * m_nPalletHeight / 200 + m_rcItem.top;
...
void CColorPaletteUI::DoEvent(TEventUI& event)
{
    ...
    if (event.Type == UIEVENT_BUTTONDOWN)
    {
        if (event.ptMouse.x >= m_rcItem.left &&
            ...
            event.ptMouse.x < m_rcItem.right &&
            event.ptMouse.y < m_rcItem.top + m_nPalletHeight)
        {
            int x = (event.ptMouse.x - m_rcItem.left) * 360 /
        ...
        if (event.ptMouse.x >= m_rcItem.left &&
            event.ptMouse.y >= m_rcItem.bottom - m_nBarHeight &&
            event.ptMouse.x < m_rcItem.right &&
            event.ptMouse.y < m_rcItem.bottom)
        {
            m_nCurS = (event.ptMouse.x - m_rcItem.left) * 200 /
                      (m_rcItem.right - m_rcItem.left);
            m_uButtonState |= UISTATE_PUSHED;
            m_bIsInBar = true;
            m_bIsInPallet = false;
...
void CColorPaletteUI::PaintPallet(HDC hDC)
{
    ...
    RECT rcCurSorPaint = {m_ptLastPalletMouse.x - 4,
                          m_ptLastPalletMouse.y - 4,
                          m_ptLastPalletMouse.x + 4,
                          m_ptLastPalletMouse.y + 4};
    CRenderEngine::DrawImageString(
        hDC, m_pManager, rcCurSorPaint, m_rcPaint, m_strThumbImage);

    rcCurSorPaint.left =
        m_rcItem.left + m_nCurS * (m_rcItem.right - m_rcItem.left) / 200 - 4;
    rcCurSorPaint.right =
        m_rcItem.left + m_nCurS * (m_rcItem.right - m_rcItem.left) / 200 + 4;
...
void CColorPaletteUI::UpdatePalletData()
{
    ...
    for (y = 0; y < 200; ++y)
    {
        for (x = 0; x < 360; ++x)
        {
            pPiexl = LPBYTE(m_pBits) + ((200 - y) * m_bmInfo.bmWidthBytes) +
                     ((x * m_bmInfo.bmBitsPixel) / 8);
    ...
    SetBitmapBits(
        m_hMemBitmap, m_bmInfo.bmWidthBytes * m_bmInfo.bmHeight, m_pBits);
}
...
void CColorPaletteUI::UpdateBarData()
{
    ...
    for (y = 0; y < m_nBarHeight; ++y)
    {
        for (x = 0; x < 200; ++x)
        {
            pPiexl = LPBYTE(m_pBits) + ((210 + y) * m_bmInfo.bmWidthBytes) +
                     ((x * m_bmInfo.bmBitsPixel) / 8);
