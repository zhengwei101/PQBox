    ...
    else if (IsEnabled())
    {
        if (event.Type == UIEVENT_MOUSEENTER && !IsAnimationRunning(FADE_IN_ID))
        {
            ...
            return;
                 ...
                 !IsAnimationRunning(FADE_OUT_ID))
        {
            m_bFadeAlpha = 0;
            m_bMouseLeave = TRUE;
...
void CFadeButtonUI::OnAnimationStart(INT nAnimationID, BOOL bFirstLoop)
{

}

void CFadeButtonUI::OnAnimationStep(INT nTotalFrame,
                                    INT nCurFrame,
                                    INT nAnimationID)
{
...
void CFadeButtonUI::OnAnimationStop(INT nAnimationID)
{
