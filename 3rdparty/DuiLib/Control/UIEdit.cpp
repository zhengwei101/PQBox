...
public:
    ...
    LRESULT
    OnEditChanged(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
...
void CEditWnd::Init(CEditUI* pOwner)
{
    ...
    SendMessage(
        EM_SETMARGINS, EC_LEFTMARGIN | EC_RIGHTMARGIN, MAKELPARAM(0, 0));
    ...
    else if (uMsg == WM_KEYDOWN && TCHAR(wParam) == VK_TAB)
    {
        if (m_pOwner->GetManager()->IsLayered())
        {
            m_pOwner->GetManager()->SetNextTabControl();
             ...
             uMsg == OCM__BASE + WM_CTLCOLORSTATIC)
    {
        ...
        if (clrColor < 0xFF000000)
        {
            ...
            HBITMAP hBmpEditBk = CRenderEngine::GenerateBitmap(
                m_pOwner->GetManager(), rcWnd, m_pOwner, clrColor);
...
LRESULT
CEditWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/,
                                WPARAM /*wParam*/,
                                LPARAM /*lParam*/,
                                BOOL& /*bHandled*/)
{
...
void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if (_tcscmp(pstrName, _T("autoselall")) == 0)
        SetAutoSelAll(_tcscmp(pstrValue, _T("true")) == 0);
