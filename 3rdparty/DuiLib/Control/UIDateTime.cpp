...
LRESULT CDateTimeWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    else if (uMsg == OCM_NOTIFY)
    {
        ...
        if (pHeader && pHeader->hwndFrom == m_hWnd)
        {
            ...
            {
                ::SendMessage(
                    m_hWnd, DTM_GETSYSTEMTIME, 0, (LPARAM)&m_pOwner->m_sysTime);
                m_pOwner->m_nDTUpdateFlag = DT_UPDATE;
                m_pOwner->UpdateText();
                PostMessage(WM_CLOSE);
                m_bDropOpen = false;
...
{
    if (lstrcmpi(pstrName, _T("timeformat")) == 0)
        SetTimeFormat(pstrValue);
    else if (lstrcmpi(pstrName, _T("readonly")) == 0)
        SetReadOny(lstrcmpi(pstrValue, _T("true")) == 0);
    else
