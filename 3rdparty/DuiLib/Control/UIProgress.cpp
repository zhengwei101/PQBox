...
void CProgressUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if (_tcsicmp(pstrName, _T("hor")) == 0)
        ...
        SetStretchForeImage(_tcsicmp(pstrValue, _T("true")) == 0 ? true
                                                                 : false);
...
void CProgressUI::PaintForeColor(HDC hDC)
{
    ...
    if (m_bHorizontal)
    {
        rc.right = m_rcItem.left + (m_nValue - m_nMin) *
                                       (m_rcItem.right - m_rcItem.left) /
                                       (m_nMax - m_nMin);
    }
    else
    {
        rc.bottom = m_rcItem.top + (m_rcItem.bottom - m_rcItem.top) *
                                       (m_nMax - m_nValue) / (m_nMax - m_nMin);
    }
...
void CProgressUI::PaintForeImage(HDC hDC)
{
    ...
    if (m_bHorizontal)
    {
        rc.right = static_cast<int>(
            (m_nValue - m_nMin) * (m_rcItem.right - m_rcItem.left) * 1.0f / (m_nMax - m_nMin));
        rc.bottom = m_rcItem.bottom - m_rcItem.top;
    }
    else
    {
        rc.top = static_cast<int>(
            (m_rcItem.bottom - m_rcItem.top) * (m_nMax - m_nValue) * 1.0f / (m_nMax - m_nMin));
    ...
    if (!m_sForeImage.IsEmpty())
    {
        ...
        int nScaleFactor = GetManager()->GetDPIObj()->GetScale();
        int sw = MulDiv(rc.right - rc.left, 100, nScaleFactor);
        int sh = MulDiv(rc.bottom - rc.top, 100, nScaleFactor);
        rc.left = MulDiv(rc.left, 100, nScaleFactor);
        rc.top = MulDiv(rc.top, 100, nScaleFactor);
        rc.right = rc.left + sw;
        rc.bottom = rc.top + sh;
        if (m_bStretchForeImage)
        {
            m_sForeImageModify.SmallFormat(
                _T("dest='%d,%d,%d,%d'"), rc.left, rc.top, rc.right, rc.bottom);
        }
        else
        {
            m_sForeImageModify.SmallFormat(
                _T("dest='%d,%d,%d,%d' source='%d,%d,%d,%d'"),
                rc.left,
                rc.top,
                rc.right,
                rc.bottom,
...
void CProgressUI::UpdateText()
{
    if (m_bShowText)
    {
        CDuiString sText;
        sText.Format(_T("%.0f%%"),
                     (m_nValue - m_nMin) * 100.0f / (m_nMax - m_nMin));
        SetText(sText);
    }
}
} // namespace DuiLib
