...
public:
    ...
    CComboUI* m_pOwner = nullptr;
    CVerticalLayoutUI* m_pLayout = nullptr;
    CVerticalLayoutUI* m_pDataLayout = nullptr;
    int m_iOldSel = -1;
    bool m_bHitItem = false;
};
    ...
    else if (msg.sType == DUI_MSGTYPE_CLICK)
    {
        ...
        while (pCtrl)
        {
            IListItemUI* pListItem =
                (IListItemUI*)pCtrl->GetInterface(DUI_CTR_LISTITEM);
            if (pListItem)
            {
                break;
            }
        ...
        if (m_pOwner->GetManager())
            m_pOwner->GetManager()->SendNotify(
                msg.pSender, DUI_MSGTYPE_CLICK, 0, 0);
    }
}
...
void CComboWnd::Init(CComboUI* pOwner)
{
    ...
    cyFixed += rcPadding.top;
    cyFixed += rcPadding.bottom;
    cyFixed += 4;
    ...
    ::MapWindowRect(hWndPaint, HWND_DESKTOP, &rc);
    ...
    Create(hWndPaint, NULL, WS_POPUP, WS_EX_TOOLWINDOW, rc);
...
bool CComboWnd::IsHitItem(POINT ptMouse)
{
    ...
    if (pControl)
    {
        ...
        {
            IListItemUI* pListItem =
                (IListItemUI*)pControl->GetInterface(DUI_CTR_LISTITEM);
            if (pListItem)
            {
                return true;
            }
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if (uMsg == WM_CREATE)
    {
        ...
        LPCTSTR pDefaultAttributes =
            m_pOwner->GetManager()->GetDefaultAttributeList(_T("VerticalLayout"));

        if (pDefaultAttributes)
        {
            m_pDataLayout->ApplyAttributeList(pDefaultAttributes);
        }

        m_pLayout->SetBkImage(m_pOwner->GetDropBoxBkImage());

        m_pDataLayout = new CVerticalLayoutUI;
        m_pLayout->Add(m_pDataLayout);

        m_pDataLayout->SetManager(&m_pm, NULL, true);
        m_pDataLayout->SetInset(CDuiRect(1, 1, 1, 1));
        m_pDataLayout->SetBkColor(0xFFFFFFFF);
        m_pDataLayout->SetAutoDestroy(false);
        m_pDataLayout->EnableScrollBar();
        m_pDataLayout->ApplyAttributeList(m_pOwner->GetDropBoxAttributeList());
        m_pDataLayout->SetPadding(m_pOwner->GetDropBoxPadding());

        for (int i = 0; i < m_pOwner->GetCount(); i++)
        {
            m_pDataLayout->Add(static_cast<CControlUI*>(m_pOwner->GetItemAt(i)));
        }
    ...
    else if (uMsg == WM_CLOSE)
    {
        ...
        m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_ENDDROPDOWN);
...
void CComboWnd::EnsureVisible(int iIndex)
{
    ...
    m_pDataLayout->FindSelectable(m_pOwner->GetCurSel(), false);
    
    RECT rcItem = m_pDataLayout->GetItemAt(iIndex)->GetPos();
    RECT rcList = m_pDataLayout->GetPos();
    CScrollBarUI* pHScrollBar = m_pDataLayout->GetHorizontalScrollBar();
    if (pHScrollBar && pHScrollBar->IsVisible())
        rcList.bottom -= pHScrollBar->GetFixedHeight();
...
void CComboWnd::Scroll(int dx, int dy)
{
    if (dx == 0 && dy == 0)
        return;
    SIZE sz = m_pDataLayout->GetScrollPos();
    m_pDataLayout->SetScrollPos(CDuiSize(sz.cx + dx, sz.cy + dy));
}
...
void CComboUI::DoEvent(TEventUI& event)
{
    ...
    if (event.Type == UIEVENT_SCROLLWHEEL)
    {
        if (GetScrollSelect())
        {
            ...
            SelectItem(
                FindSelectable(m_iCurSel + (bDownward ? 1 : -1), bDownward));
        }
        return;
    }
...
SIZE CComboUI::EstimateSize(SIZE szAvailable)
{
    if (m_cxyFixed.cy == 0)
        return CDuiSize(m_cxyFixed.cx,
                        m_pManager->GetDefaultFontInfo()->tm.tmHeight + 12);
...
void CComboUI::SetDropBoxPadding(RECT rcPadding)
{
    m_rcDropBoxPadding = rcPadding;
}
...
LPCTSTR CComboUI::GetDropBoxBkImage()
{
    return m_sDropBoxBkImage;
}

void CComboUI::SetDropBoxBkImage(LPCTSTR pStrImage)
{
    if (m_sDropBoxBkImage == pStrImage)
        return;

    m_sDropBoxBkImage = pStrImage;
}
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if (_tcsicmp(pstrName, _T("dropboxbkimage")) == 0)
        SetDropBoxBkImage(pstrValue);
    ...
    else if (_tcsicmp(pstrName, _T("dropboxpadding")) == 0)
    {
        RECT rcPadding = { 0 };
        LPTSTR pstr = NULL;
...
BOOL CComboUI::SortItems(PULVCompareFunc pfnCompare, UINT_PTR dwData)
{
    ...
    qsort_s(m_items.GetData(),
            m_items.GetSize(),
            sizeof(CControlUI*),
            CComboUI::ItemComareFunc,
            this);

    IListItemUI* pItem = NULL;
    for (int i = 0; i < m_items.GetSize(); ++i)
    {
        pItem = (IListItemUI*)(static_cast<CControlUI*>(
            m_items[i])->GetInterface(TEXT("ListItem")));
        if (pItem)
        {
            pItem->SetIndex(i);
            pItem->Select(false);
...
{
    qsort_s(m_items.GetData(), m_items.GetSize(), sizeof(CControlUI*), 
        ComareFunc, this);
...
int __cdecl CComboUI::ItemComareFunc(void* pvlocale,
                                     const void* item1,
                                     const void* item2)
{
    CComboUI* pThis = (CComboUI*)pvlocale;
