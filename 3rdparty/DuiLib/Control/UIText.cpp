...
void CTextUI::DoEvent(TEventUI& event)
{
    ...
    if (event.Type == UIEVENT_BUTTONDOWN ||
        event.Type == UIEVENT_DBLCLICK && IsEnabled())
    {
        for (int i = 0; i < m_nLinks; i++)
        {
            if (::PtInRect(&m_rcLinks[i], event.ptMouse))
...
SIZE CTextUI::EstimateSize(SIZE szAvailable)
{
    ...
    RECT rcText = {0,
                   ...
                   m_bAutoCalcWidth
                       ? 9999
                       : GetManager()->GetDPIObj()->Scale(m_cxyFixed.cx),
                   9999};
    ...
    SIZE cXY = {rcText.right - rcText.left + m_rcTextPadding.left +
                    m_rcTextPadding.right,
                rcText.bottom - rcText.top + m_rcTextPadding.top +
                    m_rcTextPadding.bottom};
