...
HRESULT STDMETHODCALLTYPE CFlashUI::GetTypeInfo(
...
HRESULT STDMETHODCALLTYPE CFlashUI::Invoke(DISPID dispIdMember,
                                           REFIID riid,
                                           LCID lcid,
                                           WORD wFlags,
                                           ...
                                           UINT* puArgErr)
{
    ...
    case DISPID_FLASHEVENT_FSCOMMAND: {
        if (pDispParams && pDispParams->cArgs == 2)
        {
            ...
            {
                return FSCommand(pDispParams->rgvarg[1].bstrVal,
...
HRESULT STDMETHODCALLTYPE CFlashUI::QueryInterface(REFIID riid,
                                                   void** ppvObject)
{
    *ppvObject = NULL;
...
LRESULT CFlashUI::TranslateAccelerator(MSG* pMsg)
{
    ...
    if (FAILED(m_pFlash->QueryInterface(IID_IOleInPlaceActiveObject,
                                        (LPVOID*)&pObj)))
        return S_FALSE;
