...
public:
    virtual void
    GetItemComboTextArray(CControlUI* pCtrl, int iItem, int iSubItem) = 0;
};
...
public:
    void OnListItemClicked(int nIndex,
                           int nColum,
                           RECT* lpRCColum,
                           LPCTSTR lpstrText);
...
public:
    BOOL
    DrawCheckBoxImage(HDC hDC, LPCTSTR pStrImage, LPCTSTR pStrModify = NULL);
    ...
    int GetCheckBoxWidth()
        const; // 实际大小位置使用GetPos获取，这里得到的是预设的参考值
...
public:
    virtual bool
    DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl);
    virtual void SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue);
    virtual void PaintStatusImage(HDC hDC);
    BOOL DrawCheckBoxImage(HDC hDC,
                           LPCTSTR pStrImage,
                           LPCTSTR pStrModify,
                           RECT& rcCheckBox);
    LPCTSTR GetCheckBoxNormalImage();
