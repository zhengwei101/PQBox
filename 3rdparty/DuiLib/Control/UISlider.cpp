...
RECT CSliderUI::GetThumbRect() const
{
    ...
    if (m_bHorizontal)
    {
        int width = m_rcItem.right - m_rcItem.left - m_szThumb.cx;
        int left = m_rcItem.left + width * (m_nValue - m_nMin) / (m_nMax - m_nMin);
        int top = (m_rcItem.bottom + m_rcItem.top - m_szThumb.cy) / 2;
        rcThumb = CDuiRect(left, top, left + m_szThumb.cx, top + m_szThumb.cy);
    }
    else
    {
        int height = m_rcItem.bottom - m_rcItem.top - m_szThumb.cy;
        int left = (m_rcItem.right + m_rcItem.left - m_szThumb.cx) / 2;
        int top = m_rcItem.bottom - m_szThumb.cy - height * (m_nValue - m_nMin) / (m_nMax - m_nMin);
        rcThumb = CDuiRect(left, top, left + m_szThumb.cx, top + m_szThumb.cy);
    }
    ...
    if (event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK)
    {
        if (IsEnabled())
        {
            ...
            if (m_bHorizontal)
            {
                ...
                {
                    float value = 1.0f * (m_nMax - m_nMin) * (event.ptMouse.x - m_rcItem.left - m_szThumb.cx / 2) / width;
                    nValue = m_nMin + static_cast<int>(value);
                }
            }
            else
            {
                ...
                {
                    float value = 1.0f * (m_nMax - m_nMin) * (m_rcItem.bottom - event.ptMouse.y - m_szThumb.cy / 2) / height;
                    nValue = m_nMin + static_cast<int>(value);
                }
            }
        ...
        if (IsEnabled())
        {
            ...
            if (m_bHorizontal)
            {
                ...
                if (event.ptMouse.x >= m_rcItem.right - m_szThumb.cx / 2)
                    ...
                    nValue = m_nMin + (m_nMax - m_nMin) *(event.ptMouse.x - m_rcItem.left - m_szThumb.cx / 2) / width;
            }
            else
            {
                int height = m_rcItem.bottom - m_rcItem.top - m_szThumb.cy;
                ...
                if (event.ptMouse.y >= m_rcItem.bottom - m_szThumb.cy / 2)
                    ...
                    nValue = m_nMin + (m_nMax - m_nMin) * (m_rcItem.bottom - event.ptMouse.y - m_szThumb.cy / 2) / height;
            }
    ...
    if (event.Type == UIEVENT_MOUSEMOVE)
    {
        if ((m_uButtonState & UISTATE_CAPTURED) != 0)
        {
            if (m_bHorizontal)
            {
                ...
                {
                    float value = 1.0f * (m_nMax - m_nMin) * (event.ptMouse.x - m_rcItem.left - m_szThumb.cx / 2) / width;
                    m_nValue = m_nMin + static_cast<int>(value);
                }   
            }
            else
            {
