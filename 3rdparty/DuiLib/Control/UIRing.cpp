...
{
    if (m_pManager)
        m_pManager->KillTimer(this, RING_TIMERID);
...
{
    if (_tcscmp(pstrName, _T("Ring")) == 0)
        return static_cast<CRingUI*>(this);
...
void CRingUI::PaintBkImage(HDC hDC)
{
    ...
    if (m_pBkimage)
    {
        ...
        Gdiplus::PointF centerPos(
            Gdiplus::REAL(rcItem.left + iWidth / 2),
            Gdiplus::REAL(rcItem.top + iHeight / 2));
        // 解决偶数时抖动问题
        ...
        graphics.DrawImage(
            m_pBkimage, rcItem.left, rcItem.top, iWidth, iHeight);
    }
}
