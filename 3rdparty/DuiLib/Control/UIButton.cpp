...
{
    if (m_dwPushedBorderColor == dwColor)
        return;
...
{
    if (m_dwDisabledBorderColor == dwColor)
        return;
...
void CButtonUI::BindTriggerTabSel(int _SetSelectIndex /*= -1*/)
{
    ...
    if (pstrName == NULL ||
        (GetBindTabLayoutIndex() < 0 && _SetSelectIndex < 0))
        return;

    CTabLayoutUI* pTabLayout =
        static_cast<CTabLayoutUI*>(GetManager()->FindControl(pstrName));
    if (!pTabLayout)
        return;
    pTabLayout->SelectItem(_SetSelectIndex >= 0 ? _SetSelectIndex
...
void CButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if (_tcsicmp(pstrName, _T("pushedforeimage")) == 0)
        SetPushedForeImage(pstrValue);
...
void CButtonUI::PaintText(HDC hDC)
{
    ...
    RECT rcTextPadding = CButtonUI::m_rcTextPadding;
    ...
    if (m_bShowHtml)
        ...
        CRenderEngine::DrawText(
            hDC, m_pManager, rc, sText, clrColor, iFont, m_uTextStyle);
}
...
void CButtonUI::PaintBkColor(HDC hDC)
{
    if ((m_uButtonState & UISTATE_DISABLED) != 0)
    {
        if (m_dwDisabledBkColor != 0)
        {
            CRenderEngine::DrawColor(
                hDC, m_rcPaint, GetAdjustColor(m_dwDisabledBkColor));
            return;
        }
    }
    else if ((m_uButtonState & UISTATE_PUSHED) != 0)
    {
        ...
        {
            CRenderEngine::DrawColor(
                hDC, m_rcPaint, GetAdjustColor(m_dwPushedBkColor));
            return;
        }
    }
    else if ((m_uButtonState & UISTATE_HOT) != 0)
    {
        ...
        {
            CRenderEngine::DrawColor(
                hDC, m_rcPaint, GetAdjustColor(m_dwHotBkColor));
            return;
        }
    }
...
void CButtonUI::PaintStatusImage(HDC hDC)
{
    if (!m_sStateImage.IsEmpty() && m_nStateCount > 0)
    {
        ...
        const TImageInfo* pImage = m_pManager->GetImageEx(info.sImageName,
                                                          info.sResType,
                                                          info.dwMask,
                                                          info.bHSL,
                                                          info.bGdiplus);
        if (m_sNormalImage.IsEmpty() && pImage)
        {
            ...
            if (szImage.cx > 0 && szImage.cy > 0)
            {
                ...
                if (m_nStateCount > 0)
                {
                    ...
                    m_sNormalImage.Format(
                        _T("res='%s' restype='%s' dest='%d,%d,%d,%d' ")
                        _T("source='%d,%d,%d,%d'"),
                        info.sImageName.GetData(),
                        info.sResType.GetData(),
                        info.rcDest.left,
                        info.rcDest.top,
                        info.rcDest.right,
                        info.rcDest.bottom,
...
void CButtonUI::PaintBorder(HDC hDC)
{
    if ((m_uButtonState & UISTATE_DISABLED) != 0)
    {
        if (m_dwDisabledBorderColor != 0)
        {
            DrawBorder(hDC,
                       m_rcItem,
                       GetAdjustColor(m_dwDisabledBorderColor),
                       m_nBorderSize,
                       m_rcBorderSize,
                       m_cxyBorderRound,
                       m_nBorderStyle);
    ...
    else if ((m_uButtonState & UISTATE_PUSHED) != 0)
    {
        if (m_dwPushedBorderColor != 0)
        {
            DrawBorder(hDC,
                       m_rcItem,
                       GetAdjustColor(m_dwPushedBorderColor),
                       m_nBorderSize,
                       m_rcBorderSize,
                       m_cxyBorderRound,
                       m_nBorderStyle);
    ...
    else if ((m_uButtonState & UISTATE_HOT) != 0)
    {
        if (m_dwHotBorderColor != 0)
        {
            DrawBorder(hDC,
                       m_rcItem,
                       GetAdjustColor(m_dwHotBorderColor),
                       m_nBorderSize,
                       m_rcBorderSize,
                       m_cxyBorderRound,
                       m_nBorderStyle);
...
void CButtonUI::DrawBorder(HDC hDC,
                           const RECT& rcItem,
    ...
    if (dwBorderColor != 0)
    {
        ...
        if (nBorderSize > 0 && (cxyBorderRound.cx > 0 || cxyBorderRound.cy > 0))
        {
            CRenderEngine::DrawRoundRect(hDC,
                                         ...
                                         cxyBorderRound.cx,
                                         cxyBorderRound.cy,
                                         GetAdjustColor(dwBorderColor),
                                         nBorderStyle);
        }
        else
        {
            if (rcBorderSize.left > 0 || rcBorderSize.top > 0 ||
