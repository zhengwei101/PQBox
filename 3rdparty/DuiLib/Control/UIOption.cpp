...
void COptionUI::SetManager(CPaintManagerUI* pManager,
                           CControlUI* pParent,
                           bool bInit)
{
        ...
        if (!m_sGroupName.IsEmpty())
        {
            if (m_bSelected)
            {
                CStdPtrArray* aOptionGroup =
                    m_pManager->GetOptionGroup(m_sGroupName);
                for (int i = 0; i < aOptionGroup->GetSize(); i++)
                {
                    COptionUI* pControl =
                        static_cast<COptionUI*>(aOptionGroup->GetAt(i));
                    if (pControl != this &&
                        (m_sGroupType.IsEmpty() ||
                         m_sGroupType.CompareNoCase(pControl->GetGroupType()) !=
                             0))
                    {
...
void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if (_tcsicmp(pstrName, _T("group")) == 0)
        SetGroup(pstrValue);
    else if (_tcsicmp(pstrName, _T("grouptype")) == 0)
        SetGroupType(pstrValue);
    ...
    else if (_tcsicmp(pstrName, _T("selectedstateimage")) == 0)
        SetSelectedStateImage(pstrValue);
    else if (_tcsicmp(pstrName, _T("selectedstatecount")) == 0)
        SetSelectedStateCount(_ttoi(pstrValue));
    ...
    if (IsSelected())
    {
        if (m_dwSelectedBkColor != 0)
        {
            CRenderEngine::DrawColor(
                hDC, m_rcPaint, GetAdjustColor(m_dwSelectedBkColor));
        }
    }
    else
    {
    ...
    if (IsSelected())
    {
        if (!m_sSelectedStateImage.IsEmpty() && m_nSelectedStateCount > 0)
        {
            ...
            const TImageInfo* pImage = m_pManager->GetImageEx(info.sImageName,
                                                              ...
                                                              info.bHSL,
                                                              info.bGdiplus);
            if (m_sSelectedImage.IsEmpty() && pImage)
            {
                SIZE szImage = {pImage->nX, pImage->nY};
                SIZE szStatus = {pImage->nX / m_nSelectedStateCount,
                                 pImage->nY};
                if (szImage.cx > 0 && szImage.cy > 0)
                {
                    ...
                    if (m_nSelectedStateCount > 0)
                    {
                        ...
                        m_sSelectedImage.Format(
                            _T("res='%s' restype='%s' dest='%d,%d,%d,%d' ")
                            _T("source='%d,%d,%d,%d' corner='%d,%d,%d,%d'"),
        ...
        if ((m_uButtonState & UISTATE_PUSHED) != 0 &&
            !m_sSelectedPushedImage.IsEmpty())
        {
...
void CCheckBoxUI::DoEvent(TEventUI& event)
{
    ...
    if (m_bAutoCheck &&
        (event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK))
    {
            ...
            if (m_bSelected)
            {
                ...
                if (bMsg)
                {
                    m_pManager->SendNotify(
                        this, DUI_MSGTYPE_SELECTCHANGED, m_bSelected, 0);
                }
            }
        }
        else
