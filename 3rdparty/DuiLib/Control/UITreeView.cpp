...
CTreeNodeUI::CTreeNodeUI(CTreeNodeUI* _ParentNode /*= NULL*/)
{
    ...
    if (_ParentNode)
    {
        ...
        pDottedLine->SetFixedWidth(
            _ParentNode->GetDottedLine()->GetFixedWidth() + 16);
...
{
    if (NULL !=
        static_cast<CTreeNodeUI*>(_pTreeNodeUI->GetInterface(_T("TreeNode"))))
...
// 参数信息: int iIndex
// 该参数仅针对当前节点下的兄弟索引，并非列表视图索引 函数说明:
//************************************
bool CTreeNodeUI::AddAt(CControlUI* pControl, int iIndex)
{
    if (NULL ==
        static_cast<CTreeNodeUI*>(pControl->GetInterface(_T("TreeNode"))))
        return FALSE;

    CTreeNodeUI* pIndexNode =
        static_cast<CTreeNodeUI*>(mTreeNodes.GetAt(iIndex));
    if (!pIndexNode)
    {
    ...
    if (!pIndexNode && pTreeView && pTreeView->GetItemAt(GetTreeIndex() + 1))
        pIndexNode =
            static_cast<CTreeNodeUI*>(pTreeView->GetItemAt(GetTreeIndex() + 1)
...
bool CTreeNodeUI::AddChildNode(CTreeNodeUI* _pTreeNodeUI)
{
    ...
    if (pTreeView)
    {
        CTreeNodeUI* pNode = static_cast<CTreeNodeUI*>(
            mTreeNodes.GetAt(mTreeNodes.GetSize() - 1));
        if (!pNode || !pNode->GetLastNode())
            nRet = pTreeView->AddAt(_pTreeNodeUI, GetTreeIndex() + 1) >= 0;
        else
            nRet =
                pTreeView->AddAt(_pTreeNodeUI,
                                 pNode->GetLastNode()->GetTreeIndex() + 1) >= 0;
    }
...
bool CTreeNodeUI::RemoveAt(CTreeNodeUI* _pTreeNodeUI)
{
    ...
    if (pNode && pNode == _pTreeNodeUI)
    {
        while (pNode->IsHasChild())
            pNode->RemoveAt(
                static_cast<CTreeNodeUI*>(pNode->mTreeNodes.GetAt(0)));
...
CTreeNodeUI* CTreeNodeUI::GetLastNode()
{
    ...
    {
        CTreeNodeUI* pNode =
            static_cast<CTreeNodeUI*>(GetTreeNodes().GetAt(nIndex));
        if (!pNode)
            continue;
        if (pNode->IsHasChild())
...
CTreeNodeUI* CTreeNodeUI::CalLocation(CTreeNodeUI* _pTreeNodeUI)
{
    ...
    _pTreeNodeUI->GetDottedLine()->SetFixedWidth(pDottedLine->GetFixedWidth() +
                                                 16);
...
bool CTreeViewUI::Add(CTreeNodeUI* pControl)
{
    ...
    pControl->GetFolderButton()->OnNotify +=
        MakeDelegate(this, &CTreeViewUI::OnFolderChanged);
    pControl->GetCheckBox()->OnNotify +=
        MakeDelegate(this, &CTreeViewUI::OnCheckBoxChanged);
    ...
    if (nFolderWidth <= 0)
        nFolderWidth = 16;
    if (!pControl->GetFolderButton()->IsVisible())
        nFolderWidth = 0;
    pControl->GetFolderButton()->SetPadding(
        CDuiRect(nLevel * nFolderWidth, 0, 0, 0));
...
// 参数信息: int iIndex
// 函数说明:
// 该方法不会将待插入的节点进行缩位处理，若打算插入的节点为非根节点，请使用AddAt(CTreeNodeUI*
// pControl,CTreeNodeUI* _IndexNode) 方法
//************************************
...
// 函数说明:
//************************************
bool CTreeViewUI::SetItemCheckBox(bool _Selected,
                                  CTreeNodeUI* _TreeNode /*= NULL*/)
{
    if (_TreeNode)
    {
...
// 函数说明:
//************************************
void CTreeViewUI::SetItemExpand(bool _Expanded,
                                CTreeNodeUI* _TreeNode /*= NULL*/)
{
    if (_TreeNode)
    {
        if (_TreeNode->GetCountChild() > 0)
        {
            ...
            for (int nIndex = 0; nIndex < nCount; nIndex++)
            {
                ...
                if (pItem->GetCountChild() &&
                    !pItem->GetFolderButton()->IsSelected())
                {
