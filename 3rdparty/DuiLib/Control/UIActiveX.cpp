...
protected:
    ...
    LRESULT
    OnMouseActivate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
...
public:
    ...
    (UINT msg, WPARAM wParam, LPARAM lParam, LRESULT* plResult);
...
protected:
    LONG m_dwRef = 1;
    CActiveXUI* m_pOwner = nullptr;
    CActiveXWnd* m_pWindow = nullptr;
    IUnknown* m_pUnkSite = nullptr;
    IViewObject* m_pViewObject = nullptr;
    IOleInPlaceObjectWindowless* m_pInPlaceObject = nullptr;

    bool m_bLocked = false;
    bool m_bFocused = false;
    bool m_bCaptured = false;
    bool m_bUIActivated = false;
    bool m_bInPlaceActive = false;
    bool m_bWindowless = true;
};
...
STDMETHODIMP
CActiveXCtrl::GetMoniker(DWORD dwAssign, DWORD dwWhichMoniker, IMoniker** ppmk)
{
...
STDMETHODIMP CActiveXCtrl::GetContainer(IOleContainer** ppContainer)
{
    ...
    *ppContainer = NULL;
    ...
    if (m_pUnkSite)
        Hr =
            m_pUnkSite->QueryInterface(IID_IOleContainer, (LPVOID*)ppContainer);
    if (FAILED(Hr))
...
STDMETHODIMP CActiveXCtrl::InvalidateRect(LPCRECT pRect, BOOL fErase)
{
    ...
    if (!m_pOwner)
        return E_UNEXPECTED;
    if (m_pOwner->m_hwndHost == NULL)
        return E_FAIL;
    return ::InvalidateRect(m_pOwner->m_hwndHost, pRect, fErase) ? S_OK
                                                                 : E_FAIL;
}
...
STDMETHODIMP
CActiveXCtrl::ScrollRect(INT dx, INT dy, LPCRECT pRectScroll, LPCRECT pRectClip)
{
...
STDMETHODIMP CActiveXCtrl::OnDefWindowMessage(UINT msg,
                                              WPARAM wParam,
                                              LPARAM lParam,
                                              LRESULT* plResult)
{
...
STDMETHODIMP CActiveXCtrl::OnInPlaceActivateEx(BOOL* pfNoRedraw, DWORD dwFlags)
{
    ...
    if (FAILED(Hr))
    {
        ...
        if (FAILED(Hr))
            return Hr;
        Hr = m_pOwner->m_pUnk->QueryInterface(IID_IOleInPlaceObject,
                                              (LPVOID*)&m_pInPlaceObject);
    }
...
STDMETHODIMP CActiveXCtrl::TransformCoords(POINTL* pPtlHimetric,
...
STDMETHODIMP CActiveXCtrl::ParseDisplayName(IBindCtx* pbc,
                                            LPOLESTR pszDisplayName,
                                            ULONG* pchEaten,
                                            IMoniker** ppmkOut)
{
...
HRESULT CActiveXCtrl::CreateActiveXWnd()
{
    ...
    m_pOwner->m_hwndHost =
        m_pWindow->Init(this, m_pOwner->GetManager()->GetPaintWindow());
    return S_OK;
}
...
void CActiveXWnd::DoVerb(LONG iVerb)
{
    ...
    CActiveXUI* pOwner = m_pOwner->m_pOwner;
    if (!pOwner)
        return;

    IOleObject* pUnk = nullptr;
    pOwner->GetControl(IID_IOleObject, (LPVOID*)&pUnk);
    ...
    pUnk->DoVerb(
        iVerb, NULL, pOleClientSite, 0, m_hWnd, &m_pOwner->m_pOwner->GetPos());
}
...
LRESULT
CActiveXWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    if (m_pOwner->m_pOwner->GetManager()->IsLayered())
    {
...
LRESULT
CActiveXWnd::OnTimer(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    if (wParam == DEFAULT_TIMERID)
    {
        if (m_pOwner->m_pOwner->GetManager()->IsLayered())
        {
            // m_pOwner->m_pOwner->GetManager()->AddNativeWindow(m_pOwner->m_pOwner,
            // m_hWnd);
            m_iLayeredTick += 1;
...
LRESULT CActiveXWnd::OnEraseBkgnd(UINT uMsg,
                                  WPARAM wParam,
                                  LPARAM lParam,
                                  BOOL& bHandled)
{
...
LRESULT CActiveXWnd::OnMouseActivate(UINT uMsg,
                                     WPARAM wParam,
                                     LPARAM lParam,
                                     BOOL& bHandled)
{
...
LRESULT
CActiveXWnd::OnSetFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    bHandled = FALSE;
...
LRESULT CActiveXWnd::OnKillFocus(UINT uMsg,
                                 WPARAM wParam,
                                 LPARAM lParam,
                                 BOOL& bHandled)
{
...
LRESULT
CActiveXWnd::OnPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    PAINTSTRUCT ps = {0};
...
LRESULT
CActiveXWnd::OnPrint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    RECT rcClient;
    ::GetClientRect(m_hWnd, &rcClient);
    m_pOwner->m_pViewObject->Draw(DVASPECT_CONTENT,
                                  -1,
                                  NULL,
                                  NULL,
                                  NULL,
    ...
    if (m_bDrawCaret)
    {
        ...
        if (guiThreadInfo.hwndCaret)
        {
            ...
            ::ScreenToClient(m_pOwner->m_pOwner->GetManager()->GetPaintWindow(),
                             &ptCaret);
...
void CActiveXUI::SetVisible(bool bVisible)
{
    ...
    if (m_hwndHost && !m_pControl->m_bWindowless)
...
void CActiveXUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    if (!m_pControl->m_bWindowless)
    {
        ...
        ::MoveWindow(*m_pControl->m_pWindow,
                     m_rcItem.left,
                     m_rcItem.top,
...
{
    if (m_pControl && m_pControl->m_bWindowless &&
        m_pControl->m_pViewObject)
    {
        m_pControl->m_pViewObject->Draw(DVASPECT_CONTENT,
                                        -1,
                                        NULL,
                                        NULL,
                                        NULL,
...
LRESULT CActiveXUI::MessageHandler(UINT uMsg,
                                   WPARAM wParam,
                                   LPARAM lParam,
                                   bool& bHandled)
{
    ...
    if ((uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_SETCURSOR)
    {
        ...
        DWORD dwHitResult =
            m_pControl->m_bCaptured ? HITRESULT_HIT : HITRESULT_OUTSIDE;
        if (dwHitResult == HITRESULT_OUTSIDE &&
            m_pControl->m_pViewObject)
        {
            IViewObjectEx* pViewEx = NULL;
            m_pControl->m_pViewObject->QueryInterface(IID_IViewObjectEx,
                                                      (LPVOID*)&pViewEx);
            if (pViewEx)
            {
                POINT ptMouse = {GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam)};
                pViewEx->QueryHitPoint(
                    DVASPECT_CONTENT, &m_rcItem, ptMouse, 0, &dwHitResult);
                pViewEx->Release();
            }
        }
    ...
    LRESULT lResult = 0;
    HRESULT Hr = m_pControl->m_pInPlaceObject->OnWindowMessage(
...
typedef HRESULT(__stdcall* DllGetClassObjectFunc)(REFCLSID rclsid,
                                                  REFIID riid,
                                                  LPVOID* ppv);
...
bool CActiveXUI::DoCreateControl()
{
    ...
    if (!m_sModuleName.IsEmpty())
    {
        ...
        if (hModule)
        {
            ...
            DllGetClassObjectFunc aDllGetClassObjectFunc =
                (DllGetClassObjectFunc)::GetProcAddress(hModule,
                                                        "DllGetClassObject");
            Hr = aDllGetClassObjectFunc(
                m_clsid, IID_IClassFactory, (LPVOID*)&aClassFactory);
            if (SUCCEEDED(Hr))
            {
                Hr = aClassFactory->CreateInstance(
                    NULL, IID_IOleObject, (LPVOID*)&pOleControl);
            }
    ...
    {
        Hr = ::CoCreateInstance(
            m_clsid, NULL, CLSCTX_ALL, IID_IOleControl, (LPVOID*)&pOleControl);
    }
    ASSERT(SUCCEEDED(Hr));
    if (FAILED(Hr))
        return false;
    ...
    m_pUnk->QueryInterface(IID_IPersistStreamInit,
                           (LPVOID*)&pPersistStreamInit);
    ...
    if ((dwMiscStatus & OLEMISC_SETCLIENTSITEFIRST) == 0)
        m_pUnk->SetClientSite(pOleClientSite);
    // Grab the view...
    Hr = m_pUnk->QueryInterface(IID_IViewObjectEx,
                                (LPVOID*)&m_pControl->m_pViewObject);
    if (FAILED(Hr))
        Hr = m_pUnk->QueryInterface(IID_IViewObject2,
                                    (LPVOID*)&m_pControl->m_pViewObject);
    if (FAILED(Hr))
        Hr = m_pUnk->QueryInterface(IID_IViewObject,
                                    (LPVOID*)&m_pControl->m_pViewObject);
    // Activate and done...
    m_pUnk->SetHostNames(OLESTR("UIActiveX"), NULL);
    if (m_pManager)
        m_pManager->SendNotify(
            (CControlUI*)this, DUI_MSGTYPE_SHOWACTIVEX, 0, 0, false);
    if ((dwMiscStatus & OLEMISC_INVISIBLEATRUNTIME) == 0)
    {
        try
        {
            if (m_pManager)
                Hr = m_pUnk->DoVerb(OLEIVERB_INPLACEACTIVATE,
                                    NULL,
                                    pOleClientSite,
                                    0,
