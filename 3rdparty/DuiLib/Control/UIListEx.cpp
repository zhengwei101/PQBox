...
{
    if (_tcsicmp(pstrName, _T("ListEx")) == 0)
        return static_cast<IListOwnerUI*>(this);
    return CListUI::GetInterface(pstrName);
}
BOOL CListExUI::CheckColumEditable(int nColum)
{
    CListContainerHeaderItemUI* pHItem =
        static_cast<CListContainerHeaderItemUI*>(m_pHeader->GetItemAt(nColum));
...
CRichEditUI* CListExUI::GetEditUI()
{
    if (m_pEditUI == NULL)
    {
        ...
        LPCTSTR pDefaultAttributes =
            GetManager()->GetDefaultAttributeList(_T("RichEdit"));
...
CComboBoxUI* CListExUI::GetComboBoxUI()
{
    if (m_pComboBoxUI == NULL)
    {
        ...
        LPCTSTR pDefaultAttributes =
            GetManager()->GetDefaultAttributeList(_T("Combo"));
        if (pDefaultAttributes)
        {
...
BOOL CListExUI::CheckColumCheckBoxable(int nColum)
{
    ...
    CListContainerHeaderItemUI* pHItem =
        static_cast<CListContainerHeaderItemUI*>(
            p->GetInterface(_T("ListContainerHeaderItem")));
...
void CListExUI::Notify(TNotifyUI& msg)
{
    ...
    if (_tcsicmp(msg.sType, _T("listheaditemchecked")) == 0)
    {
        ...
        for (int i = 0; i < pHeader->GetCount(); i++)
        {
            ...
            {
                for (int j = 0; j < GetCount(); ++i)
                {
                    CControlUI* p = GetItemAt(j);
                    CListTextExtElementUI* pLItem =
                        static_cast<CListTextExtElementUI*>(
                            p->GetInterface(_T("ListTextExElement")));
                    if (pLItem)
                    {
    ...
    else if (_tcsicmp(msg.sType, DUI_MSGTYPE_LISTITEMCHECKED) == 0)
    {
        for (int i = 0; i < GetCount(); ++i)
        {
            ...
            CListTextExtElementUI* pLItem = static_cast<CListTextExtElementUI*>(
                p->GetInterface(_T("ListTextExElement")));
    ...
    //编辑框、组合框
    if (_tcsicmp(strName, _T("ListEx_Edit")) == 0 && m_pEditUI && m_nRow >= 0 &&
        m_nColum >= 0)
    {
        ...
        else if (_tcsicmp(msg.sType, DUI_MSGTYPE_KILLFOCUS) == 0)
        {
            ...
            CListTextExtElementUI* pRowCtrl =
                (CListTextExtElementUI*)GetItemAt(m_nRow);
            if (pRowCtrl)
            {
             ...
             m_nRow >= 0 && m_nColum >= 0)
    {
        ...
        else if (_tcsicmp(msg.sType, DUI_MSGTYPE_LISTITEMSELECT) == 0 &&
                 iOldSel >= 0)
        {
            CListTextExtElementUI* pRowCtrl =
                (CListTextExtElementUI*)GetItemAt(m_nRow);
...
void CListExUI::OnListItemClicked(int nIndex,
                                  int nColum,
                                  RECT* lpRCColum,
                                  LPCTSTR lpstrText)
{
    ...
    else
    {
        if (CheckColumEditable(nColum) && GetEditUI())
        {
            ...
            SIZE szTextSize =
                CRenderEngine::GetTextSize(m_pManager->GetPaintDC(),
                                           m_pManager,
                                           _T("TTT"),
                                           m_ListInfo.nFont,
                                           DT_CALCRECT | DT_SINGLELINE);
            m_pEditUI->SetTextPadding(CDuiRect(
                2,
                (lpRCColum->bottom - lpRCColum->top - szTextSize.cy) / 2,
                2,
                0));
            //设置文字
        ...
        else if (CheckColumComboBoxable(nColum) && GetComboBoxUI())
        {
            ...
            if (m_pXCallback)
            {
                m_pXCallback->GetItemComboTextArray(
                    m_pComboBoxUI, nIndex, nColum);
            }
...
void CListExUI::OnListItemChecked(int nIndex, int nColum, BOOL bChecked)
{
    ...
    if (bChecked)
    {
        ...
        for (int i = 0; i < GetCount(); i++)
        {
            CControlUI* pp = GetItemAt(i);
            CListTextExtElementUI* pLItem = static_cast<CListTextExtElementUI*>(pp->GetInterface(_T("ListTextExElement")));
...
{
    if (_tcsicmp(pstrName, _T("ListContainerHeaderItem")) == 0)
        return this;
...
{
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(m_iSepWidth);
    return m_iSepWidth;
}
...
void CListContainerHeaderItemUI::SetAttribute(LPCTSTR pstrName,
                                              LPCTSTR pstrValue)
{
    ...
    else if (_tcsicmp(pstrName, _T("textpadding")) == 0)
    {
        ...
        SetColumeEditable(_tcsicmp(pstrValue, _T("true")) == 0);
    else if (_tcsicmp(pstrName, _T("comboable")) == 0)
        SetColumeComboable(_tcsicmp(pstrValue, _T("true")) == 0);
    else if (_tcsicmp(pstrName, _T("checkable")) == 0)
        SetColumeCheckable(_tcsicmp(pstrValue, _T("true")) == 0);
    else if (_tcsicmp(pstrName, _T("checkboxwidth")) == 0)
        SetCheckBoxWidth(_ttoi(pstrValue));
    else if (_tcsicmp(pstrName, _T("checkboxheight")) == 0)
        SetCheckBoxHeight(_ttoi(pstrValue));
    else if (_tcsicmp(pstrName, _T("checkboxnormalimage")) == 0)
        SetCheckBoxNormalImage(pstrValue);
    else if (_tcsicmp(pstrName, _T("checkboxhotimage")) == 0)
        SetCheckBoxHotImage(pstrValue);
    else if (_tcsicmp(pstrName, _T("checkboxpushedimage")) == 0)
        SetCheckBoxPushedImage(pstrValue);
    else if (_tcsicmp(pstrName, _T("checkboxfocusedimage")) == 0)
        SetCheckBoxFocusedImage(pstrValue);
    else if (_tcsicmp(pstrName, _T("checkboxdisabledimage")) == 0)
        SetCheckBoxDisabledImage(pstrValue);
    else if (_tcsicmp(pstrName, _T("checkboxselectedimage")) == 0)
        SetCheckBoxSelectedImage(pstrValue);
    else if (_tcsicmp(pstrName, _T("checkboxforeimage")) == 0)
        SetCheckBoxForeImage(pstrValue);
...
void CListContainerHeaderItemUI::DoEvent(TEventUI& event)
{
                ...
                if (::PtInRect(&rcCheckBox, event.ptMouse))
                {
                    ...
                    if (pOwner)
                    {
                        m_pManager->SendNotify(this,
                                               DUI_MSGTYPE_LISTHEADITEMCHECKED,
                                               pOwner->GetItemIndex(this),
                                               m_bChecked);
                    }
                }
                m_uCheckBoxState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
    ...
    if (event.Type == UIEVENT_SETCURSOR)
    {
        ...
        if (m_iSepWidth >= 0)
            ...
            rcSeparator.right += 4;
        if (IsEnabled() && m_bDragable &&
            ::PtInRect(&rcSeparator, event.ptMouse))
        {
...
{
    if (m_cxyFixed.cy == 0)
        return CDuiSize(m_cxyFixed.cx,
                        m_pManager->GetDefaultFontInfo()->tm.tmHeight + 14);
...
{
    if (m_iSepWidth >= 0)
        return CDuiRect(m_rcItem.right - m_iSepWidth,
                        m_rcItem.top,
                        m_rcItem.right,
                        m_rcItem.bottom);
    else
        return CDuiRect(m_rcItem.left,
                        m_rcItem.top,
                        m_rcItem.left - m_iSepWidth,
                        m_rcItem.bottom);
}
...
void CListContainerHeaderItemUI::PaintStatusImage(HDC hDC)
{
    ...
    if (m_bCheckBoxable)
    {
        do
        {
            ...
            if (IsFocused())
                m_uCheckBoxState |= UISTATE_FOCUSED;
            else
                m_uCheckBoxState &= ~UISTATE_FOCUSED;

            if (!IsEnabled())
                m_uCheckBoxState |= UISTATE_DISABLED;
            else
                m_uCheckBoxState &= ~UISTATE_DISABLED;
...
void CListContainerHeaderItemUI::PaintText(HDC hDC)
{
    ...
    int nLinks = 0;
    if (m_bShowHtml)
        CRenderEngine::DrawHtmlText(hDC,
                                    m_pManager,
                                    rcText,
                                    sText,
                                    m_dwTextColor,
...
void CListContainerHeaderItemUI::SetCheck(BOOL bCheck)
{
    ...
    m_bChecked = bCheck;
    if (m_bChecked)
        m_uCheckBoxState |= UISTATE_SELECTED;
    else
        m_uCheckBoxState &= ~UISTATE_SELECTED;
    Invalidate();
}
...
BOOL CListContainerHeaderItemUI::DrawCheckBoxImage(HDC hDC,
                                                   LPCTSTR pStrImage,
                                                   LPCTSTR pStrModify)
{
    RECT rcCheckBox;
    ...
    return CRenderEngine::DrawImageString(
        hDC, m_pManager, rcCheckBox, m_rcPaint, pStrImage, pStrModify);
}
...
{
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(m_cxyCheckBox.cx);
...
{
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(m_cxyCheckBox.cy);
...
{
    if (_tcsicmp(pstrName, _T("ListTextExElement")) == 0)
        return static_cast<CListTextExtElementUI*>(this);
...
UINT CListTextExtElementUI::GetControlFlags() const
{
    return UIFLAG_WANTRETURN |
           ((IsEnabled() && m_nLinks > 0) ? UIFLAG_SETCURSOR : 0);
}
            ...
            else if (event.Type == UIEVENT_BUTTONUP)
            {
                if ((m_uCheckBoxState & UISTATE_CAPTURED) != 0)
                {
                    if (::PtInRect(&rcCheckBox, event.ptMouse))
                    {
                        ...
                        {
                            m_pManager->SendNotify(this,
                                                   DUI_MSGTYPE_LISTITEMCHECKED,
                                                   MAKEWPARAM(GetIndex(), 0),
                                                   m_bChecked);
                        }
                    }
                    m_uCheckBoxState &= ~(UISTATE_PUSHED | UISTATE_CAPTURED);
...
void CListTextExtElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for (int i = 0; i < pInfo->nColumns; i++)
    {
        ...
        {
            CRenderEngine::DrawColor(hDC, rc, iTextBkColor);
        }

        rc.left += pInfo->rcTextPadding.left;
        rc.right -= pInfo->rcTextPadding.right;
        rc.top += pInfo->rcTextPadding.top;
        rc.bottom -= pInfo->rcTextPadding.bottom;
        ...
        if (pListCtrl->CheckColumCheckBoxable(i))
        {
            RECT rcCheckBox;
            GetCheckBoxRect(i, rcCheckBox);
            rc.left += (rcCheckBox.right - rcCheckBox.left);
        }
    ...
    for (int i = 0; i < pInfo->nColumns; i++)
    {
        if (pListCtrl->CheckColumCheckBoxable(i))
        {
            ...
            if ((m_uCheckBoxState & UISTATE_SELECTED) != 0)
            {
                if (!m_sCheckBoxSelectedImage.IsEmpty())
                {
                    if (!DrawCheckBoxImage(hDC,
                                           (LPCTSTR)m_sCheckBoxSelectedImage,
                                           NULL,
                                           rcCheckBox))
                    {
                    }
            ...
            if ((m_uCheckBoxState & UISTATE_DISABLED) != 0)
            {
                ...
                {
                    if (!DrawCheckBoxImage(hDC,
                                           (LPCTSTR)m_sCheckBoxDisabledImage,
                                           NULL,
                                           rcCheckBox))
                    {
                    }
            ...
            else if ((m_uCheckBoxState & UISTATE_PUSHED) != 0)
            {
                ...
                {
                    if (!DrawCheckBoxImage(hDC,
                                           (LPCTSTR)m_sCheckBoxPushedImage,
                                           NULL,
                                           rcCheckBox))
                    {
                    }
            ...
            else if ((m_uCheckBoxState & UISTATE_FOCUSED) != 0)
            {
                ...
                {
                    if (!DrawCheckBoxImage(hDC,
                                           (LPCTSTR)m_sCheckBoxFocusedImage,
                                           NULL,
                                           rcCheckBox))
                    {
                    }
            ...
            if (!m_sCheckBoxNormalImage.IsEmpty())
            {
                if (!DrawCheckBoxImage(
                        hDC, (LPCTSTR)m_sCheckBoxNormalImage, NULL, rcCheckBox))
                {
                }
                else
                    return;
        ...
        Label_ForeImage:
            ...
            {
                if (!DrawCheckBoxImage(
                        hDC, (LPCTSTR)m_sCheckBoxForeImage, NULL, rcCheckBox))
                {
                }
            }
        }
...
BOOL CListTextExtElementUI::DrawCheckBoxImage(HDC hDC,
                                              LPCTSTR pStrImage,
                                              LPCTSTR pStrModify,
                                              RECT& rcCheckBox)
{
...
{
    if (_tcsicmp(pstrName, _T("checkboxwidth")) == 0)
        SetCheckBoxWidth(_ttoi(pstrValue));
    else if (_tcsicmp(pstrName, _T("checkboxheight")) == 0)
        SetCheckBoxHeight(_ttoi(pstrValue));
    else if (_tcsicmp(pstrName, _T("checkboxnormalimage")) == 0)
...
bool CListTextExtElementUI::DoPaint(HDC hDC,
                                    const RECT& rcPaint,
...
int CListTextExtElementUI::HitTestColum(POINT ptMouse)
{
    ...
    for (int i = 0; i < pInfo->nColumns; i++)
    {
        RECT rcItem = {pInfo->rcColumn[i].left,
                       m_rcItem.top,
                       pInfo->rcColumn[i].right,
                       m_rcItem.bottom};
