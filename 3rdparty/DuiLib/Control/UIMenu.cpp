...
LRESULT
CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT
CMenuWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT
CMenuWnd::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
                             ...
                             CControlUI* pStopControl)
{
    ...
    if (m_bDrawLine)
    {
        RECT rcLine = {m_rcItem.left + rcLinePadding.left,
                       m_rcItem.top + cxyFixed.cy / 2,
...
void CMenuElementUI::DrawItemExpland(HDC hDC, const RECT& rcItem)
{
    if (m_bShowExplandIcon)
    {
        ...
        RECT rcDest = {cxyFixed.cx - pImageInfo->nX - padding,
...
SIZE CMenuElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if (cXY.cy == 0)
    {
        ...
        {
            int nLinks = 0;
            CRenderEngine::DrawHtmlText(m_pManager->GetPaintDC(),
                                        m_pManager,
                                        rcText,
                                        sText,
                                        iTextColor,
...
SIZE CMenuElementUI::GetIconSize()
{
    ...
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(&szIconSize);
    return szIconSize;
}
