...
void CHotKeyWnd::Init(CHotKeyUI* pOwner)
{
    ...
    do
    {
        ...
        HWND hWnd = Create(
            m_pOwner->GetManager()->GetPaintWindow(), NULL, uStyle, 0, rcPos);
        if (!IsWindow(hWnd))
        {
            break;
        }
        SetWindowFont(
            m_hWnd,
            m_pOwner->GetManager()->GetFontInfo(m_pOwner->GetFont())->hFont,
            TRUE);
...
RECT CHotKeyWnd::CalPos()
{
    ...
    LONG lHeight =
        m_pOwner->GetManager()->GetFontInfo(m_pOwner->GetFont())->tm.tmHeight;
...
LRESULT CHotKeyWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    else if (uMsg == WM_PAINT)
    {
        ...
        HBRUSH hBrush = CreateSolidBrush(RGB(
            GetBValue(dwBkColor), GetGValue(dwBkColor), GetRValue(dwBkColor)));
        ::FillRect(hDC, &rect, hBrush);
        ::DeleteObject(hBrush);
        HFONT hOldFont = (HFONT)SelectObject(hDC, GetWindowFont(m_hWnd));
        ::SIZE size = {0};
        ::GetTextExtentPoint32(
            hDC, strText.GetData(), strText.GetLength(), &size);
        ::DrawText(hDC,
                   strText.GetData(),
                   -1,
                   &rect,
...
LRESULT
CHotKeyWnd::OnKillFocus(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CHotKeyWnd::OnEditChanged(UINT uMsg,
                                  WPARAM wParam,
                                  LPARAM lParam,
                                  BOOL& bHandled)
{
...
void CHotKeyWnd::SetHotKey(WORD wVirtualKeyCode, WORD wModifiers)
{
    ...
    ::SendMessage(
        m_hWnd, HKM_SETHOTKEY, MAKEWORD(wVirtualKeyCode, wModifiers), 0L);
}
...
void CHotKeyWnd::SetRules(WORD wInvalidComb, WORD wModifiers)
{
    ASSERT(::IsWindow(m_hWnd));
    ::SendMessage(
        m_hWnd, HKM_SETRULES, wInvalidComb, MAKELPARAM(wModifiers, 0));
}
...
CDuiString CHotKeyWnd::GetKeyName(UINT vk, BOOL fExtended)
{
    ...
    switch (vk)
    {
        // Keys which are "extended" (except for Return which is Numeric Enter
        // as extended)
    case VK_INSERT:
    case VK_DELETE:
    case VK_HOME:
...
{
    if (_tcscmp(pstrName, _T("HotKey")) == 0)
        return static_cast<CHotKeyUI*>(this);
...
SIZE CHotKeyUI::EstimateSize(SIZE szAvailable)
{
    if (m_cxyFixed.cy == 0)
        return CDuiSize(m_cxyFixed.cx,
                        m_pManager->GetFontInfo(GetFont())->tm.tmHeight + 6);
