                                   ...
                                   CControlUI* pParent)
{
    ...
    if (HIWORD(xml.m_lpstr) && *(xml.m_lpstr) != _T('<'))
    {
        LPCTSTR xmlpath =
            CResourceManager::GetInstance()->GetXmlPath(xml.m_lpstr);
        if (xmlpath)
        {
            xml = xmlpath;
        }
    ...
    else
    {
        ...
        if (!m_xml.LoadFromMem((BYTE*)::LockResource(hGlobal),
            ::SizeofResource(dll_instence, hResource)))
        {
            return nullptr;
        }
...
CControlUI* CDialogBuilder::Create(IDialogBuilderCallback* pCallback,
                                   CPaintManagerUI* pManager,
                                   CControlUI* pParent)
{
    m_pCallback = pCallback;
    ...
    if (pManager)
    {
             ...
             node = node.GetSibling())
        {
            ...
            if (_tcsicmp(pstrClass, _T("Image")) == 0)
            {
                ...
                if (pImageName)
                    pManager->AddImage(
                        pImageName, pImageResType, mask, false, shared);
            }
            else if (_tcsicmp(pstrClass, _T("Font")) == 0)
            {
                ...
                {
                    pManager->AddFont(
                        id, pFontName, size, bold, underline, italic, shared);
                    if (defaultfont)
                        pManager->SetDefaultFont(
                            pFontName, size, bold, underline, italic, shared);
                }
            }
            else if (_tcsicmp(pstrClass, _T("Default")) == 0)
            {
                ...
                if (pControlName)
                {
                    pManager->AddDefaultAttributeList(
                        pControlName, pControlValue, shared);
                }
            }
            ...
            if (pManager->GetPaintWindow())
            {
                ...
                for (int i = 0; i < nAttributes; i++)
                {
                    ...
                    if (_tcsicmp(pstrName, _T("size")) == 0)
                    {
                        ...
                        ASSERT(pstr);
                        pManager->SetInitSize(pManager->GetDPIObj()->Scale(cx),
                                              pManager->GetDPIObj()->Scale(cy));
                    }
                    ...
                    {
                        pManager->GetShadow()->ShowShadow(
                            _tcsicmp(pstrValue, _T("true")) == 0);
                    }
                    else if (_tcsicmp(pstrName, _T("gdiplustext")) == 0)
                    {
                        pManager->SetUseGdiplusText(
                            _tcsicmp(pstrValue, _T("true")) == 0);
                    }
...
void CDialogBuilder::GetLastErrorMessage(LPTSTR pstrMessage,
                                         SIZE_T cchMax) const
{
    return m_xml.GetLastErrorMessage(pstrMessage, cchMax);
}

void CDialogBuilder::GetLastErrorLocation(LPTSTR pstrSource,
                                          SIZE_T cchMax) const
{
    return m_xml.GetLastErrorLocation(pstrSource, cchMax);
}

void CDialogBuilder::SetInstance(HINSTANCE instance)
{
    m_instance = instance;
}

CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot,
                                   CControlUI* pParent,
                                   CPaintManagerUI* pManager)
{
    IContainerUI* pContainer = NULL;
    CControlUI* pReturn = NULL;
    for (CMarkupNode node = pRoot->GetChild(); node.IsValid();
         node = node.GetSibling())
    {
        ...
        if (_tcsicmp(pstrClass, _T("Include")) == 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                { // 使用资源dll，从资源中读取
                    ...
                    pControl = builder.Create(
                        (UINT)id, m_pstrtype, m_pCallback, pManager, pParent);
                }
                else
                {
                    pControl = builder.Create((LPCTSTR)szValue,
                                              nullptr,
                                              m_pCallback,
                                              pManager,
                                              pParent);
        ...
        else
        {
            ...
            pControl = dynamic_cast<CControlUI*>(
        ...
        if (pParent && pControl)
        {
            CTreeNodeUI* pParentTreeNode = static_cast<CTreeNodeUI*>(
                pParent->GetInterface(_T("TreeNode")));
            CTreeNodeUI* pTreeNode = static_cast<CTreeNodeUI*>(
                pControl->GetInterface(_T("TreeNode")));
            pTreeView = static_cast<CTreeViewUI*>(
                pParent->GetInterface(_T("TreeView")));
            // TreeNode子节点
            if (pTreeNode)
            {
                if (pParentTreeNode)
            ...
            {
                if (pContainer == NULL)
                    pContainer = static_cast<IContainerUI*>(
                        pParent->GetInterface(_T("IContainer")));
                ASSERT(pContainer);
                if (pContainer == NULL)
                    return nullptr;
        ...
        if (pManager)
        {
            ...
            LPCTSTR pDefaultAttributes =
                pManager->GetDefaultAttributeList(pstrClass);
        ...
        if (node.HasAttributes())
        {
            ...
            for (int i = 0; i < nAttributes; i++)
            {
                pControl->SetAttribute(node.GetAttributeName(i),
