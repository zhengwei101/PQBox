...
#define MSGDEF(x)                                                                                  \
    ...
    ::swprintf(szMsg, 10, _T("0x%04X"), uMsg);
    return szMsg;
}
...
{
    HICON hIcon =
        (HICON)::LoadImage(CPaintManagerUI::GetInstance(),
...
bool CWindowWnd::RegisterWindowClass()
{
    ...
    const auto ret = ::RegisterClass(&wc);
...
bool CWindowWnd::RegisterSuperclass()
{
    if (m_bUnicode)
    {
        LPCTSTR lpSuperClassName = GetSuperClassName();
        LPCTSTR lpClassName = GetWindowClassName();
        ...
        const auto ret = ::RegisterClassEx(&wc);
        ASSERT(ret != NULL || ::GetLastError() == ERROR_CLASS_ALREADY_EXISTS);
        return ret != NULL || ::GetLastError() == ERROR_CLASS_ALREADY_EXISTS;
    }
    else
...
LRESULT
CWindowWnd::SendMessage(UINT uMsg, WPARAM wParam /*= 0*/, LPARAM lParam /*= 0*/)
{
    ASSERT(::IsWindow(m_hWnd));
...
LRESULT
CWindowWnd::PostMessage(UINT uMsg, WPARAM wParam /*= 0*/, LPARAM lParam /*= 0*/)
{
    ASSERT(::IsWindow(m_hWnd));
