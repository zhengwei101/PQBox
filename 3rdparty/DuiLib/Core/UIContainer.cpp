    ...
    if (bEnableVertical && !m_pVerticalScrollBar)
    {
        ...
        if (m_pManager)
        {
            LPCTSTR pDefaultAttributes =
                m_pManager->GetDefaultAttributeList(_T("VScrollBar"));
...
void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    {
        EnableScrollBar(_tcsicmp(pstrValue, _T("true")) == 0,
            (GetHorizontalScrollBar() != nullptr));
    }
    ...
    {
        EnableScrollBar((GetVerticalScrollBar() != nullptr),
    ...
    else if (_tcsicmp(pstrName, _T("fixedscrollbar")) == 0)
        SetFixedScrollbar(_tcsicmp(pstrValue, _T("true")) == 0);
    else if (_tcsicmp(pstrName, _T("showscrollbar")) == 0)
        SetShowScrollbar(_tcsicmp(pstrValue, _T("true")) == 0);
    else
                              ...
                              bool bInit)
{
    for (int it = 0; it < m_items.GetSize(); it++)
    {
        static_cast<CControlUI*>(m_items[it])
            ->SetManager(pManager, this, bInit);
    }
...
CControlUI*
CContainerUI::FindControl(FINDCONTROLPROC Proc, LPVOID pData, UINT uFlags)
{
    ...
    if ((uFlags & UIFIND_HITTEST) != 0 &&
        ...
        return nullptr;
    if ((uFlags & UIFIND_UPDATETEST) != 0 && Proc(this, pData))
        return nullptr;
    ...
    if (pResult == NULL && m_pVerticalScrollBar)
    {
        if ((uFlags & UIFIND_HITTEST) == 0 || IsMouseEnabled())
            pResult = m_pVerticalScrollBar->FindControl(Proc, pData, uFlags);
    }
    if (pResult == NULL && m_pHorizontalScrollBar)
    {
        if ((uFlags & UIFIND_HITTEST) == 0 || IsMouseEnabled())
    ...
    if ((uFlags & UIFIND_HITTEST) == 0 || IsMouseChildEnabled())
    {
        ...
        if ((uFlags & UIFIND_TOP_FIRST) != 0)
        {
            ...
            {
                pResult = static_cast<CControlUI*>(m_items[it])
                              ->FindControl(Proc, pData, uFlags);
                if (pResult)
                {
...
bool CContainerUI::DoPaint(HDC hDC,
                           const RECT& rcPaint,
...
void CContainerUI::SetFloatPos(int iIndex)
{
    ...
    else
    {
        ...
        rcCtrl.bottom =
            (LONG)(height * rcPercent.bottom) + szXY.cy + sz.cy + top;
        pControl->SetPos(rcCtrl, false);
    }
}
...
void CContainerUI::ProcessScrollBar(RECT rc, int cxRequired, int cyRequired)
{
    while (m_pHorizontalScrollBar)
    {
            ...
            !m_pHorizontalScrollBar->IsVisible())
        {
            ...
            m_pHorizontalScrollBar->SetScrollRange(cxRequired -
                                                   (rc.right - rc.left));
    ...
    while (m_pVerticalScrollBar)
    {
            ...
            !m_pVerticalScrollBar->IsVisible())
        {
            ...
            m_pVerticalScrollBar->SetScrollRange(cyRequired -
                                                 (rc.bottom - rc.top));
        ...
        // Scroll not needed anymore?
        const int cyScroll = cyRequired - (rc.bottom - rc.top);
        if (cyScroll <= 0)
        {
        ...
        const RECT rcScrollBarPos = {rc.right,
                                     rc.top, 
                                     rc.right + m_pVerticalScrollBar->GetFixedWidth(), 
                                     rc.bottom};
        ...
        {
            const int iScrollPos = m_pVerticalScrollBar->GetScrollPos();
...
{
    return GetManager()->FindSubControlByName(this, pstrSubControlName);
}
