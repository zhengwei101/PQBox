...
void CControlUI::SetManager(CPaintManagerUI* pManager,
                            CControlUI* pParent,
                            bool bInit)
{
    m_pManager = pManager;
...
void CControlUI::SetInstance(HINSTANCE instance /*= nullptr*/)
{
    m_instance = instance;
}
...
{
    if (m_pManager)
        return m_pManager->GetDPIObj()->Scale(m_nBorderSize);
...
RECT CControlUI::GetBorderRectSize() const
{
    ...
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(&rcBorderSize);
    return rcBorderSize;
}
...
SIZE CControlUI::GetBorderRound() const
{
    ...
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(&cxyBorderRound);
...
{
    return CRenderEngine::DrawImageString(hDC,
                                          m_pManager,
                                          m_rcItem,
                                          m_rcPaint,
                                          pStrImage,
...
RECT CControlUI::GetPadding() const
{
    ...
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(&rcPadding);
    return rcPadding;
}
...
SIZE CControlUI::GetFixedXY() const
{
    SIZE cXY = m_cXY;
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(&cXY);
    return cXY;
}
...
int CControlUI::GetMinWidth() const
{
    ...
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(&cxyMin);
    return cxyMin.cx;
}
...
int CControlUI::GetMaxWidth() const
{
    ...
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(&cxyMax);
    return cxyMax.cx;
}
...
{
    if (pstrName == NULL || pstrName[0] == _T('\0') || pstrAttr == NULL ||
        pstrAttr[0] == _T('\0'))
        return;
...
LPCTSTR CControlUI::GetCustomAttribute(LPCTSTR pstrName) const
{
    ...
    CDuiString* pCostomAttr =
        static_cast<CDuiString*>(m_mCustomAttrHash.Find(pstrName));
    if (pCostomAttr)
...
{
    // 样式
    if (m_pManager && _tcsicmp(pstrName, _T("style")) == 0)
    {
    ...
    else if (_tcsicmp(pstrName, _T("pos")) == 0)
    {
        ...
        ASSERT(pstr);
        SIZE szXY = {rcPos.left >= 0 ? rcPos.left : rcPos.right,
    ...
    else if (_tcsicmp(pstrName, _T("bkcolor")) == 0 ||
             _tcsicmp(pstrName, _T("bkcolor1")) == 0)
    {
        while (*pstrValue > _T('\0') && *pstrValue <= _T(' '))
            pstrValue = ::CharNext(pstrValue);
        if (*pstrValue == _T('#'))
    ...
    else if (_tcsicmp(pstrName, _T("richevent")) == 0)
        SetRichEvent(_tcsicmp(pstrValue, _T("true")) == 0);
...
bool CControlUI::Paint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
{
    ...
    if (!::IntersectRect(&m_rcPaint, &rcPaint, &m_rcItem))
        return true;
    if (!DoPaint(hDC, m_rcPaint, pStopControl))
        return false;
    return true;
}
...
bool CControlUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
{
    ...
    if (cxyBorderRound.cx > 0 || cxyBorderRound.cy > 0)
    {
        ...
        CRenderClip::GenerateRoundClip(hDC,
                                       m_rcPaint,
                                       m_rcItem,
                                       cxyBorderRound.cx,
                                       cxyBorderRound.cy,
    ...
    if (m_dwBackColor != 0)
    {
        ...
        if (m_dwBackColor2 != 0)
        {
            if (m_dwBackColor3 != 0)
            {
                ...
                CRenderEngine::DrawGradient(hDC,
                                            rc,
                                            GetAdjustColor(m_dwBackColor),
                                            GetAdjustColor(m_dwBackColor2),
                                            bVer,
                                            8);
                rc.top = rc.bottom;
            ...
            CRenderEngine::DrawColor(
                hDC, m_rcPaint, GetAdjustColor(m_dwBackColor));
        else
            CRenderEngine::DrawColor(
                hDC, m_rcItem, GetAdjustColor(m_dwBackColor));
    }
}
...
void CControlUI::PaintBorder(HDC hDC)
{
    ...
    if (m_dwBorderColor != 0 || m_dwFocusBorderColor != 0)
    {
        ...
        else
        {
            ...
            {
                CRenderEngine::DrawRect(hDC,
                                        m_rcItem,
                                        nBorderSize,
                                        GetAdjustColor(m_dwFocusBorderColor),
                                        m_nBorderStyle);
            }
            else if (rcBorderSize.left > 0 || rcBorderSize.top > 0 ||
                     rcBorderSize.right > 0 || rcBorderSize.bottom > 0)
            {
                RECT rcBorder;
