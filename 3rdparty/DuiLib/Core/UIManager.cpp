...
struct TIMERINFO
{
    ...
    UINT
        nLocalID; //当前 Timer 的 id，并不是传入到 ::SettTimer 中的 id，而是调用 bool CPaintManagerUI::SetTimer(CControlUI* pControl, UINT nTimerID, UINT uElapse) 时的第 2 个参数
    HWND hWnd;    //当前 Timer 对应的窗口句柄，是实际传入到 ::SetTimer 中的参数
    UINT uWinTimer; //实际的 Timer 的 Id
    bool
...
void TDrawInfo::Parse(LPCTSTR pStrImage, LPCTSTR pStrModify, CPaintManagerUI* pManager)
{
    ...
    // corner='0,0,0,0' mask='#FF0000' fade='255' hole='false' xtiled='false'
    // ytiled='false'
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        while (*pStrImage != _T('\0'))
        {
            ...
            if (!sValue.IsEmpty())
            {
                ...
                {
                    szImage.cx = _tcstol(sValue.GetData(), &pstr, 10);
                    ASSERT(pstr);
                ...
                {
                    rcPadding.left = _tcstol(sValue.GetData(), &pstr, 10);
                    ASSERT(pstr);
                    ...
                    ASSERT(pstr);
                    if (pManager)
                        pManager->GetDPIObj()->Scale(&rcPadding);
                }
            }
            if (*pStrImage++ != _T(' '))
                break;
    ...
    // 调整DPI资源
    if (pManager && pManager->GetDPIObj()->GetScale() != 100)
    {
        CDuiString sScale;
...
typedef BOOL(__stdcall* PFUNCUPDATELAYEREDWINDOW)(HWND,
                                                  HDC,
                                                  POINT*,
                                                  SIZE*,
                                                  HDC,
...
UILIB_RESTYPE CPaintManagerUI::m_resType = UILIB_RESTYPE::FILE;
    ...
    , m_iHoverTime(400UL)
{
    if (m_sharedResInfo.m_DefaultFontInfo.sFontName.IsEmpty())
    {
        ...
        ::ZeroMemory(&m_sharedResInfo.m_DefaultFontInfo.tm,
                     sizeof(m_sharedResInfo.m_DefaultFontInfo.tm));
    }
...
void CPaintManagerUI::SetResourceType(UILIB_RESTYPE nType)
{
    m_resType = nType;
}

UILIB_RESTYPE CPaintManagerUI::GetResourceType()
{
    return m_resType;
}
...
CPaintManagerUI* CPaintManagerUI::GetPaintManager(LPCTSTR pstrName)
{
    ...
    {
        CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
        if (pManager && sName == pManager->GetName())
            return pManager;
    }
    return nullptr;
}
...
bool CPaintManagerUI::LoadPlugin(LPCTSTR pstrModuleName)
{
    ...
    if (hModule)
    {
        LPCREATECONTROL lpCreateControl
            = (LPCREATECONTROL)::GetProcAddress(hModule, "CreateControl");
        if (lpCreateControl)
        {
...
void CPaintManagerUI::SetOpacity(BYTE nOpacity)
{
    ...
    if (m_hWndPaint)
    {
        ...
        if (hUser32)
        {
            fSetLayeredWindowAttributes = (PFUNCSETLAYEREDWINDOWATTR)::GetProcAddress(
                hUser32, "SetLayeredWindowAttributes");
...
bool CPaintManagerUI::PreMessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes)
{
    ...
    {
        bool bHandled = false;
        //这里调用接口IMessageFilterUI::MessageHandler来进行消息过滤
        LRESULT lResult = static_cast<IMessageFilterUI*>(m_aPreMessageFilters[i])
                              ->MessageHandler(uMsg, wParam, lParam, bHandled);
        if (bHandled)
        {
            lRes = lResult;
            return true;
    ...
    switch (uMsg)
    {
        case WM_KEYDOWN:
        {
            ...
            if (wParam == VK_TAB)
            {
                if (m_pFocus && m_pFocus->IsVisible() && m_pFocus->IsEnabled()
                    && _tcsstr(m_pFocus->GetClass(), _T("RichEditUI")))
                {
                ...
                if (m_pFocus && m_pFocus->IsVisible() && m_pFocus->IsEnabled()
                    && _tcsstr(m_pFocus->GetClass(), _T("WkeWebkitUI")))
                {
                    return false;
                }
        ...
        case WM_SYSCHAR:
        {
            ...
            CControlUI* pControl
                = m_pRoot->FindControl(__FindControlFromShortcut,
                                       &fs,
...
bool CPaintManagerUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, LRESULT& lRes)
{
    ...
    {
        bool bHandled = false;
        LRESULT lResult = static_cast<IMessageFilterUI*>(m_aMessageFilters[i])
                              ->MessageHandler(uMsg, wParam, lParam, bHandled);
    ...
    switch (uMsg)
    {
        case WM_APP + 1:
        {
            ...
            TNotifyUI* pMsg = NULL;
            while ((pMsg = static_cast<TNotifyUI*>(m_aAsyncNotify.GetAt(0))) != nullptr)
            {
        ...
        case WM_PAINT:
        {
            ...
            //	if( rcPaint.bottom > rcClient.bottom ) rcPaint.bottom =
            //rcClient.bottom;
            //	::ZeroMemory(&m_rcLayeredUpdate, sizeof(m_rcLayeredUpdate));
            // }
            // else {
            ...
            if (m_bUpdateNeeded)
            {
                ...
                if (!::IsRectEmpty(&rcClient) && !::IsIconic(m_hWndPaint))
                {
                    ...
                    else
                    {
                        ...
                        m_pRoot->FindControl(__FindControlsFromUpdate,
                                             NULL,
            ...
            else if (m_bLayered && m_bLayeredChanged)
            {
                RECT rcRoot = rcClient;
                if (m_pOffscreenBits)
                    ::ZeroMemory(m_pOffscreenBits,
                                 (rcRoot.right - rcRoot.left) * (rcRoot.bottom - rcRoot.top) * 4);
                rcRoot.left += m_rcLayeredInset.left;
                rcRoot.top += m_rcLayeredInset.top;
                rcRoot.right -= m_rcLayeredInset.right;
                rcRoot.bottom -= m_rcLayeredInset.bottom;
            ...
            if (m_bOffscreenPaint && m_hbmpOffscreen == NULL)
            {
                ...
                m_hbmpOffscreen = CRenderEngine::CreateARGB32Bitmap(
                    m_hDcPaint, dwWidth, dwHeight, (LPBYTE*)&m_pOffscreenBits);
            ...
            if (m_bOffscreenPaint)
            {
                ...
                if (m_bLayered)
                {
                    for (LONG y = rcClient.bottom - rcPaint.bottom;
                         y < rcClient.bottom - rcPaint.top;
                         ++y)
                    {
                ...
                if (m_bLayered)
                {
                    for (int i = 0; i < m_aNativeWindow.GetSize();)
                    {
                        ...
                        HBITMAP hChildBitmap
                            = CRenderEngine::CreateARGB32Bitmap(hChildMemDC,
                                                                rcChildWnd.right - rcChildWnd.left,
                                                                rcChildWnd.bottom - rcChildWnd.top,
                                                                (BYTE**)&pChildBitmapBits);

                        ::ZeroMemory(pChildBitmapBits,
                                     (rcChildWnd.right - rcChildWnd.left)
                                         * (rcChildWnd.bottom - rcChildWnd.top) * 4);

                        HBITMAP hOldChildBitmap
                            = (HBITMAP)::SelectObject(hChildMemDC, hChildBitmap);
                        ...
                        for (LONG y = 0; y < rcChildWnd.bottom - rcChildWnd.top; y++)
                        {
                            ...
                            {
                                pChildBitmapBit = pChildBitmapBits
                                                  + y * (rcChildWnd.right - rcChildWnd.left) + x;
                                if (*pChildBitmapBit != 0x00000000)
                                    *pChildBitmapBit |= 0xff000000;
                            }
                        }
                        ::BitBlt(m_hDcOffscreen,
                                 rcChildWnd.left,
                                 rcChildWnd.top,
                                 rcChildWnd.right - rcChildWnd.left,
                                 rcChildWnd.bottom - rcChildWnd.top,
                ...
                {
                    CControlUI* pPostPaintControl
                        = static_cast<CControlUI*>(m_aPostPaintControls[i]);
                ...
                if (m_bLayered)
                {
                    ...
                    {
                        DWORD width = rcClient.right - rcClient.left;
                        DWORD height = rcClient.bottom - rcClient.top;
                        ...
                        if (!m_diLayered.sDrawString.IsEmpty())
                        {
                            if (m_hbmpBackground == NULL)
                            {
                                ...
                                m_hbmpBackground = CRenderEngine::CreateARGB32Bitmap(
                                    m_hDcPaint, width, height, (BYTE**)&m_pBackgroundBits);
                                ::ZeroMemory(m_pBackgroundBits, dwWidth * dwHeight * 4);
                                ::SelectObject(m_hDcBackground, m_hbmpBackground);
                                CRenderClip clip;
                                CRenderClip::GenerateClip(m_hDcBackground, rcLayeredClient, clip);
                                CRenderEngine::DrawImageInfo(m_hDcBackground,
                                                             this,
                                                             rcLayeredClient,
                                                             rcLayeredClient,
                                                             &m_diLayered);
                                 ...
                                 ++y)
                            {
                                ...
                                {
                                    pOffscreenBits
                                        = (COLORREF*)(m_pOffscreenBits + y * dwWidth + x);
                    ...
                    else
                    {
                             ...
                             ++y)
                        {
                            ...
                            {
                                int i = (y * dwWidth + x) * 4;
                                if ((m_pOffscreenBits[i + 3] == 0)
                                    && (m_pOffscreenBits[i + 0] != 0 || m_pOffscreenBits[i + 1] != 0
                                        || m_pOffscreenBits[i + 2] != 0))
                    ...
                    POINT ptSrc = {0, 0};
                    g_fUpdateLayeredWindow(m_hWndPaint,
                                           m_hDcPaint,
                                           &ptPos,
                                           &sizeWnd,
                                           m_hDcOffscreen,
                ...
                if (m_bShowUpdateRect && !m_bLayered)
                {
                    ...
                    ::Rectangle(
                        m_hDcPaint, rcPaint.left, rcPaint.top, rcPaint.right, rcPaint.bottom);
        ...
        case WM_PRINTCLIENT:
        {
            ...
            if ((lParam & PRF_CHILDREN) != 0)
            {
                ...
                while (hWndChild)
                {
                    ...
                    ::MapWindowPoints(
                        HWND_DESKTOP, m_hWndPaint, reinterpret_cast<LPPOINT>(&rcPos), 2);
        ...
        case WM_MOUSELEAVE:
        {
            ...
            if (m_bMouseTracking)
            {
                ...
                ::GetCursorPos(&pt);
                ::GetWindowRect(m_hWndPaint, &rcWnd);
                if (!::IsIconic(m_hWndPaint) && ::GetActiveWindow() == m_hWndPaint
                    && ::PtInRect(&rcWnd, pt))
                {
                    if (::SendMessage(m_hWndPaint, WM_NCHITTEST, 0, MAKELPARAM(pt.x, pt.y))
                        == HTCLIENT)
                    {
                        ::ScreenToClient(m_hWndPaint, &pt);
        ...
        case WM_MOUSEMOVE:
        {
            ...
            if (pNewHover && pNewHover->GetManager() != this)
                break;
            ...
            if (bNeedDrag && m_bDragMode && wParam == MK_LBUTTON)
            {
                ...
                ::ReleaseCapture();
                // 接口
                if (m_pDragDrop && m_pDragDrop->OnDragDrop(m_pEventClick))
                {
                    m_bDragMode = false;
                    break;
                }
            ...
            if (!IsCaptured())
            {
                ...
                if (pNewHover != m_pEventHover && m_pEventHover)
                {
                    ...
                    ::CopyMemory(aNeedMouseLeaveNeeded.GetData(),
                                 m_aNeedMouseLeaveNeeded.GetData(),
        ...
        case WM_MOUSEWHEEL:
        {
            ...
            // Let's make sure that the scroll item below the cursor is the same as
            // before...
            ::SendMessage(m_hWndPaint,
                          WM_MOUSEMOVE,
                          0,
                          (LPARAM)MAKELPARAM(m_ptLastMousePos.x, m_ptLastMousePos.y));
        }
        break;
        case WM_CHAR:
        {
...
bool CPaintManagerUI::AttachDialog(CControlUI* pControl)
{
    ...
    m_pEventClick = NULL;
    m_pEventRClick = NULL;
    // Remove the existing control-tree. We might have gotten inside this
    // function as a result of an event fired or similar, so we cannot just
    // delete the objects and pull the internal memory of the calling code.
    // We'll delay the cleanup.
    if (m_pRoot)
    {
...
void DuiLib::CPaintManagerUI::ResetDPIAssets()
{
    ...
    {
        TFontInfo* pFontInfo
            = static_cast<TFontInfo*>(m_ResInfo.m_CustomFonts.Find(m_ResInfo.m_CustomFonts[it]));
    ...
    {
        TFontInfo* pFontInfo = static_cast<TFontInfo*>(
            m_sharedResInfo.m_CustomFonts.Find(m_sharedResInfo.m_CustomFonts[it]));
        RebuildFont(pFontInfo);
    }
...
void CPaintManagerUI::SetFocus(CControlUI* pControl)
{
    ...
    if (pControl && pControl->GetManager() == this && pControl->IsVisible()
        && pControl->IsEnabled())
    {
        m_pFocus = pControl;
        TEventUI event = {0};
...
void CPaintManagerUI::SetFocusNeeded(CControlUI* pControl)
{
    ...
    info.bForward = false;
    m_pFocus = m_pRoot->FindControl(
        __FindControlFromTab, &info, UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
    m_bFocusNeeded = true;
    if (m_pRoot)
        m_pRoot->NeedUpdate();
}
...
bool CPaintManagerUI::SetNextTabControl(bool bForward)
{
    ...
    CControlUI* pControl = m_pRoot->FindControl(
        __FindControlFromTab, &info1, UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
    if (pControl == NULL)
    {
        if (bForward)
        {
            ...
            pControl = m_pRoot->FindControl(
                __FindControlFromTab, &info2, UIFIND_VISIBLE | UIFIND_ENABLED | UIFIND_ME_FIRST);
        }
        else
        {
...
void CPaintManagerUI::SendNotify(CControlUI* pControl,
                                 LPCTSTR pstrMessage,
                                 WPARAM wParam /*= 0*/,
                                 LPARAM lParam /*= 0*/,
                                 bool bAsync /*= false*/)
...
void CPaintManagerUI::SetDefaultLinkHoverFontColor(DWORD dwColor, bool bShared)
{
    if (bShared)
    {
        if (m_ResInfo.m_dwDefaultLinkHoverFontColor
            == m_sharedResInfo.m_dwDefaultLinkHoverFontColor)
...
TFontInfo* CPaintManagerUI::GetDefaultFontInfo()
{
    if (m_ResInfo.m_DefaultFontInfo.sFontName.IsEmpty())
    {
        ...
        {
            HFONT hOldFont
                = (HFONT)::SelectObject(m_hDcPaint, m_sharedResInfo.m_DefaultFontInfo.hFont);
...
void CPaintManagerUI::SetDefaultFont(LPCTSTR pStrFontName,
                                     int nSize,
                                     bool bBold,
                                     bool bUnderline,
                                     bool bItalic,
...
HFONT CPaintManagerUI::AddFont(int id,
                               LPCTSTR pStrFontName,
                               int nSize,
                               bool bBold,
                               bool bUnderline,
                               bool bItalic,
                               bool bShared)
{
    ...
    {
        TFontInfo* pOldFontInfo
            = static_cast<TFontInfo*>(m_sharedResInfo.m_CustomFonts.Find(idBuffer));
        if (pOldFontInfo)
        {
...
void CPaintManagerUI::AddFontArray(LPCTSTR pstrPath)
{
    ...
    while (!pData)
    {
        ...
        HANDLE hFile = ::CreateFile(pstrPath,
                                    GENERIC_READ,
                                    FILE_SHARE_READ,
                                    NULL,
                                    OPEN_EXISTING,
...
HFONT CPaintManagerUI::GetFont(LPCTSTR pStrFontName,
                               int nSize,
                               bool bBold,
                               bool bUnderline,
                               bool bItalic)
...
int CPaintManagerUI::GetFontIndex(LPCTSTR pStrFontName,
                                  int nSize,
                                  bool bBold,
                                  bool bUnderline,
                                  bool bItalic,
...
const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR bitmap,
                                              LPCTSTR type,
                                              DWORD mask,
                                              bool bUseHSL,
                                              bool bGdiplus,
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap,
                                            LPCTSTR type,
                                            DWORD mask,
                                            bool bUseHSL,
                                            bool bGdiplus,
                                            ...
                                            HINSTANCE instance)
{
    ...
    if (type && lstrlen(type) > 0)
    {
        if (isdigit(*bitmap))
        {
            ...
            data = bGdiplus ? CRenderEngine::GdiplusLoadImage(iIndex, type, mask, instance)
                            : CRenderEngine::LoadImage(iIndex, type, mask, instance);
        }
    }
    else
    {
        data = bGdiplus ? CRenderEngine::GdiplusLoadImage(bitmap, NULL, mask, instance)
                        : CRenderEngine::LoadImage(bitmap, NULL, mask, instance);
        if (!data)
        {
            ...
            if (iAtIdx != -1 && iDotIdx != -1)
            {
                ...
                data = bGdiplus
                           ? CRenderEngine::GdiplusLoadImage(
                               sImageName.GetData(), NULL, mask, instance)
                           : CRenderEngine::LoadImage(sImageName.GetData(), NULL, mask, instance);
            }
        }
    }
    ...
    if (data)
    {
        if (bShared || m_bForceUseSharedRes)
        {
            TImageInfo* pOldImageInfo
                = static_cast<TImageInfo*>(m_sharedResInfo.m_ImageHash.Find(bitmap));
            if (pOldImageInfo)
            {
        ...
        {
            TImageInfo* pOldImageInfo
                = static_cast<TImageInfo*>(m_ResInfo.m_ImageHash.Find(bitmap));
            if (pOldImageInfo)
            {
                CRenderEngine::FreeImage(pOldImageInfo);
            ...
            if (data)
            {
                if (!data->sResType.IsEmpty())
                {
                    if (isdigit(*bitmap))
                    {
                        LPTSTR pstr = NULL;
                        int iIndex = _tcstol(bitmap, &pstr, 10);
                        pNewData = CRenderEngine::LoadImage(
                            iIndex, data->sResType.GetData(), data->dwMask);
                    }
                }
                else
                {
...
const TDrawInfo* CPaintManagerUI::GetDrawInfo(LPCTSTR pStrImage, LPCTSTR pStrModify)
{
    CDuiString sKey;
    sKey.Format(_T("%s%s"),
                pStrImage == NULL ? _T("") : pStrImage,
                pStrModify == NULL ? _T("") : pStrModify);
...
void CPaintManagerUI::AddDefaultAttributeList(LPCTSTR pStrControlName,
                                              ...
                                              bool bShared)
{
    if (bShared || m_bForceUseSharedRes)
    {
        ...
        if (pDefaultAttr)
        {
            CDuiString* pOldDefaultAttr = static_cast<CDuiString*>(
...
bool CPaintManagerUI::RemoveDefaultAttributeList(LPCTSTR pStrControlName, bool bShared)
{
    if (bShared)
    {
        CDuiString* pDefaultAttr
            = static_cast<CDuiString*>(m_sharedResInfo.m_AttrHash.Find(pStrControlName));
        if (!pDefaultAttr)
            return false;
    ...
    {
        CDuiString* pDefaultAttr
            = static_cast<CDuiString*>(m_ResInfo.m_AttrHash.Find(pStrControlName));
        if (!pDefaultAttr)
            return false;
...
CControlUI* CPaintManagerUI::FindControl(POINT pt) const
{
    ...
    return m_pRoot->FindControl(
        __FindControlFromPoint, &pt, UIFIND_VISIBLE | UIFIND_HITTEST | UIFIND_TOP_FIRST);
}
...
CControlUI* CPaintManagerUI::FindSubControlByClass(CControlUI* pParent,
                                                   LPCTSTR pstrClass,
                                                   int iIndex)
{
    if (pParent == NULL)
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromShortcut(CControlUI* pThis, LPVOID pData)
{
    ...
    if (pFS->ch == toupper(pThis->GetShortcut()))
        pFS->bPickNext = true;
    if (_tcsstr(pThis->GetClass(), _T("LabelUI")))
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromClass(CControlUI* pThis, LPVOID pData)
{
    ...
    if (pFoundControls->GetAt(pFoundControls->GetSize() - 1))
        return pThis;
    return nullptr;
}
...
bool CPaintManagerUI::TranslateAccelerator(LPMSG pMsg)
{
    ...
    {
        LRESULT lResult = static_cast<ITranslateAccelerator*>(m_aTranslateAccelerator[i])
...
bool CPaintManagerUI::TranslateMessage(const LPMSG pMsg)
{
    ...
    if (uChildRes != 0)
    {
        ...
        for (int i = 0; i < m_aPreMessages.GetSize(); i++)
        {
            auto pT = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
...
bool CPaintManagerUI::RemoveTranslateAccelerator(ITranslateAccelerator* pTranslateAccelerator)
{
    for (int i = 0; i < m_aTranslateAccelerator.GetSize(); i++)
    {
        if (static_cast<ITranslateAccelerator*>(m_aTranslateAccelerator[i])
...
bool CPaintManagerUI::OnDrop(FORMATETC* pFmtEtc, STGMEDIUM& medium, DWORD* pdwEffect)
{
    ...
    if (pFmtEtc->cfFormat == CF_DIB && medium.tymed == TYMED_HGLOBAL)
    {
        if (medium.hGlobal)
        {
            ...
            if (lpbi)
            {
                ...
                if (hbm)
                    hbm = (HBITMAP)SendMessage(
                        m_hTargetWnd, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hbm);
                if (hbm)
                    DeleteObject(hbm);
    ...
    if (pFmtEtc->cfFormat == CF_BITMAP && medium.tymed == TYMED_GDI)
    {
        if (medium.hBitmap)
        {
            HBITMAP hBmp = (HBITMAP)SendMessage(
                m_hTargetWnd, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)medium.hBitmap);
            if (hBmp)
                DeleteObject(hBmp);
    ...
    if (pFmtEtc->cfFormat == CF_ENHMETAFILE && medium.tymed == TYMED_ENHMF)
    {
        ...
        RECT rc; //={0,0,EnhMetaHdr.rclBounds.right-EnhMetaHdr.rclBounds.left,
                 //EnhMetaHdr.rclBounds.bottom-EnhMetaHdr.rclBounds.top};
        HDC hDC = GetDC(m_hTargetWnd);
        // start code: taken from ENHMETA.EXE MSDN Sample
        //*ALSO NEED to GET the pallete (select and RealizePalette it, but i was
        //too lazy*
        ...
        if (hDrop)
        {
            ...
            if (cFiles > 0)
            {
                ...
                HBITMAP hBitmap = (HBITMAP)LoadImage(
                    NULL, szFileName, IMAGE_BITMAP, 0, 0, LR_DEFAULTSIZE | LR_LOADFROMFILE);
                if (hBitmap)
                {
                    HBITMAP hBmp = (HBITMAP)SendMessage(
                        m_hTargetWnd, STM_SETIMAGE, IMAGE_BITMAP, (LPARAM)hBitmap);
                    if (hBmp)
                        DeleteObject(hBmp);
                }
            }
