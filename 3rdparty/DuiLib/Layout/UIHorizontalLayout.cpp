...
void CHorizontalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    for (int it2 = 0; it2 < m_items.GetSize(); it2++)
    {
        ...
        if (iEstimate > 1)
            cxFixedRemaining = cxFixedRemaining - iChildPadding;
        SIZE sz = pControl->EstimateSize(szControlAvailable);
        if (sz.cx == 0)
        {
            iAdjustable++;
    ...
    if (m_iSepWidth != 0)
    {
        ...
        if (event.Type == UIEVENT_MOUSEMOVE)
        {
            if ((m_uButtonState & UISTATE_CAPTURED) != 0)
            {
                ...
                {
                    if (cx > 0 &&
                        event.ptMouse.x < m_rcNewPos.right - m_iSepWidth)
                        return;
                ...
                else
                {
                    if (cx > 0 && event.ptMouse.x < m_rcNewPos.left)
                        return;
                    if (cx < 0 &&
                        event.ptMouse.x > m_rcNewPos.left - m_iSepWidth)
                        return;
                    rc.left += cx;
                    if (rc.right - rc.left <= GetMinWidth())
                    {
...
RECT CHorizontalLayoutUI::GetThumbRect(bool bUseNew) const
{
    ...
    {
        if (m_iSepWidth >= 0)
            return CDuiRect(m_rcNewPos.right - m_iSepWidth,
                            m_rcNewPos.top,
                            m_rcNewPos.right,
                            m_rcNewPos.bottom);
        else
