...
void CVerticalLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    for (int it2 = 0; it2 < m_items.GetSize(); it2++)
    {
        ...
        if (iEstimate > 1)
            cyFixedRemaining = cyFixedRemaining - iChildPadding;
        SIZE sz = pControl->EstimateSize(szControlAvailable);
        if (sz.cy == 0)
        {
            iAdjustable++;
        ...
        else if (iChildAlign == DT_RIGHT)
        {
            ...
            RECT rcCtrl = {iPosX - rcPadding.right - sz.cx,
                           iPosY + rcPadding.top,
                           iPosX - rcPadding.right,
                           iPosY + sz.cy + rcPadding.top};
            pControl->SetPos(rcCtrl, false);
        }
        else
...
{
    if (m_pManager)
        m_pManager->GetDPIObj()->Scale(m_iSepHeight);
    return m_iSepHeight;
}
    ...
    if (m_iSepHeight != 0)
    {
        ...
        if (event.Type == UIEVENT_MOUSEMOVE)
        {
            if ((m_uButtonState & UISTATE_CAPTURED) != 0)
            {
                ...
                {
                    if (cy > 0 &&
                        event.ptMouse.y < m_rcNewPos.bottom + m_iSepHeight)
                        return;
                    ...
                    {
                        if (m_rcNewPos.bottom - m_rcNewPos.top <=
                            GetMinHeight())
                            return;
                ...
                else
                {
                    if (cy > 0 && event.ptMouse.y < m_rcNewPos.top)
                        return;
                    if (cy < 0 &&
                        event.ptMouse.y > m_rcNewPos.top + m_iSepHeight)
                        return;
                    rc.top += cy;
                    if (rc.bottom - rc.top <= GetMinHeight())
                    {
                ...
                m_rcNewPos = rc;
                m_cxyFixed.cy = GetManager()->GetDPIObj()->Scale(
                    m_rcNewPos.bottom - m_rcNewPos.top);
    ...
    else
    {
        if (m_iSepHeight >= 0)
            return CDuiRect(m_rcItem.left,
                            MAX(m_rcItem.bottom - m_iSepHeight, m_rcItem.top),
                            m_rcItem.right,
                            m_rcItem.bottom);
        else
            return CDuiRect(m_rcItem.left,
