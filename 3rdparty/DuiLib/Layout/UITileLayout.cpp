...
void CTileLayoutUI::SetPos(RECT rc, bool bNeedInvalidate)
{
    ...
    if (szItem.cx > 0)
    {
        m_nColumns = (rc.right - rc.left) / szItem.cx;
    }
    ...
    {
        cxWidth =
            (rc.right - rc.left - m_pHorizontalScrollBar->GetScrollRange()) /
            m_nColumns;
    }
    ...
    for (int it1 = 0; it1 < m_items.GetSize(); it1++)
    {
        ...
        if ((iCount % m_nColumns) == 0)
        {
            ...
            {
                CControlUI* pLineControl =
                    static_cast<CControlUI*>(m_items[it2]);
                ...
                SIZE szAvailable = {rcTile.right - rcTile.left -
                ...
                cyHeight =
                    MAX(cyHeight, szTile.cy + rcPadding.top + rcPadding.bottom);
        ...
        if (szTile.cy > pControl->GetMaxHeight())
            szTile.cy = pControl->GetMaxHeight();
        RECT rcPos = {(rcTile.left + rcTile.right - szTile.cx) / 2,
                      (rcTile.top + rcTile.bottom - szTile.cy) / 2,
                      (rcTile.left + rcTile.right - szTile.cx) / 2 + szTile.cx,
                      (rcTile.top + rcTile.bottom - szTile.cy) / 2 + szTile.cy};
